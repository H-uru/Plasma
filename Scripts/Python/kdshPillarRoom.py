...
class kdshPillarRoom(ptResponder):
    ...
    def OnServerInitComplete(self):
        ...
        if (
            ageSDL["pheight01"][0] == 1
            and ageSDL["pheight02"][0] == 4
            and ageSDL["pheight03"][0] == 1
            and ageSDL["pheight04"][0] == 2
            and ageSDL["PillarRoomSolved"][0] != 1
    ...
    def OnNotify(self, state, id, events):
        ...
        if id in [
            ...
            4,
            5,
        ]:  # One of the four levers or the reset ring were clicked by a player
            ...
            if self.PillarIsSafeToMove(
                ...
                if ageSDL["budget"][0] == 0:  # true if 8 "raises" have already happened
                    ...
                    ageSDL["pheight0" + str(id - 5)][0] == 4
                ):  # True if that particular pillar has already been raised 4 times
            ...
            for pillarcheck in [
                ...
                8,
                9,
            ]:  # using 6-9 because those ID numbers correspond with the callbacks after the "pull lever" oneshot, as if the lever was pulled

                if self.PillarIsSafeToMove(pillarcheck):
                    pass
                else:
                    PtDebugPrint(
                        "kdshPillar.OnNotify: A reset button was pushed, but Pillar",
                        pillarcheck - 4,
                        "can't reset now.",
                    )
                    safetoreset = False
    ...
    def OnSDLNotify(self, VARname, SDLname, playerID, tag):
        ...
        if VARname == "budget" or VARname == "PillarsOccupied":
            ...
            if Resetting:
                ...
                if ageSDL["budget"][0] != 8:
                    ...
                    CounterAnim.value.speed(
                        2 * (float(sumofpulls) / float(highest))
                    )  # ensures counterweights will be reset by the same time, or before, the highest pillar
            ...
            if (
                newpheight == 0
            ):  # if height is now 0, don't move it with RaiseAPillar, because ResetPuzzle has already taken care of it
                return
    ...
    def DisableAppropriateLadders(
        self, pillar
    ):  # only the ladders boxes affected by raising this pillar are disabled during movement
    ...
    def EnableAppropriateLadders(
        self, id
    ):  # when pillar is done raising, enable appropriate ladder boxes with new lengths
        ageSDL = PtGetAgeSDL()
        ...
        if (
            ...
            tolerance02 = 5 - (
                id + 1
            )  # Pillar01 ladder is 4 notches high, Pillar02 ladder is 3 notches high, pillar03 ladder is 2 notches high, pillar04 is 1 notch high. Note that we're concerned about the NEXT pillar's ladder, hence the id+1.
