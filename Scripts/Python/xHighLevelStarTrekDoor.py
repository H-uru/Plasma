...
class xHighLevelStarTrekDoor(ptModifier):
    ...
    def OnServerInitComplete(self):
        ...
        if strDoorEnabledVar.value:
            try:
                ...
                self.DoorEnabled = 1
                PtDebugPrint(
                    "ERROR: xHighLevelStarTrekDoor.OnServerInitComplete():\tERROR: age sdl read failed, defaulting door enabled value"
                )
        else:
            self.DoorEnabled = 1
        ...
        PtDebugPrint(
            "xHighLevelStarTrekDoor: Player List = %d" % len(PtGetPlayerList())
        )

        if len(PtGetPlayerList()) > 0:

            PtDebugPrint(
                "xHighLevelStarTrekDoor: Somebody is already in the age. Attempting to sync states."
            )

            if (
                self.DoorState == doorSDLstates["opening"]
                or self.DoorState == doorSDLstates["movingopen"]
                or self.DoorState == doorSDLstates["opentoclose"]
            ):
                ...
                self.DoorState == doorSDLstates["closing"]
                or self.DoorState == doorSDLstates["movingclosed"]
                or self.DoorState == doorSDLstates["closetoopen"]
            ):
    ...
    def OnSDLNotify(self, VARname, SDLname, playerID, tag):
        ...
        if VARname == strDoorEnabledVar.value:
            self.DoorEnabled = ageSDL[strDoorEnabledVar.value][0]
            PtDebugPrint(
                "HighLevelStarTrekDoor.OnSDLNotify: updated doorEnabled to %d"
                % self.DoorEnabled
            )
        elif VARname == strDoorClosedVar.value:
            doorClosed = ageSDL[strDoorClosedVar.value][0]
            PtDebugPrint(
                "HighLevelStarTrekDoor.OnSDLNotify: Door Closed SDL Updated to: %d"
                % doorClosed
            )
            PtDebugPrint(
                "HighLevelStarTrekDoor.OnSDLNotify: Player who updated SDL: ", playerID
            )
    ...
    def OnNotify(self, state, id, events):
        ...
        if id == (-1):
            if (
                events[0][1].find("rgnTriggerEnter") != -1
                and self.sceneobject.isLocallyOwned()
            ):
                PtDebugPrint(
                    "xHighLevelStarTrekDoor: Avatar who entered the region. ",
                    events[0][1].lstrip("rgnTriggerEnter"),
                )
                if self.DoorState == doorSDLstates["closed"]:
                    ...
                    self.DoorState == doorSDLstates["movingclosed"]
                    or self.DoorState == doorSDLstates["closing"]
                ):
                ...
                events[0][1].find("rgnTriggerExit") != -1
                and self.sceneobject.isLocallyOwned()
            ):
                PtDebugPrint(
                    "xHighLevelStarTrekDoor: Avatar who exited the region. ",
                    events[0][1].lstrip("rgnTriggerExit"),
                )
                if self.DoorState == doorSDLstates["open"]:
                    ...
                    self.DoorState == doorSDLstates["movingopen"]
                    or self.DoorState == doorSDLstates["opening"]
                ):
                    ...
                    PtDebugPrint(
                        "xHighLevelStarTrekDoor: Setting Door to movingclosed."
                    )
                return

            elif (
                events[0][1].find("respOpenDoor") != -1
                or events[0][1].find("respCloseDoor") != -1
            ):
                self.DoorStack.append(events[0][1])
                PtDebugPrint(
                    "xHighLevelStarTrekDoor: New list is: %s" % (str(self.DoorStack))
                )

                if len(self.DoorStack) == 1:
                    PtDebugPrint(
                        "xHighLevelStarTrekDoor: List is only one command long, so I'm playing it"
                    )
                ...
                events[0][1].find("DoorState") != -1
                and events[0][1].find("rgnTriggerEnter") == -1
                and events[0][1].find("rgnTriggerExit") == -1
                and events[0][1].find("Responder") == -1
            ):
                # PtDebugPrint("xHighLevelStarTrekDoor: Events = ", events[0][1].lstrip('DoorState='))
                curState = int(events[0][1].lstrip("DoorState="))
                PtDebugPrint(
                    "xHighLevelStarTrekDoor: Door State Updated to %d" % curState
                )
                ...
                if curState != self.DoorState:
                    ...
                    PtDebugPrint(
                        "xHighLevelStarTrekDoor: Door state is now %d" % self.DoorState
                    )
                return
    ...
    def UpdateRespStack(self):
        ...
        if len(self.DoorStack):
            PtDebugPrint(
                "xHighLevelStarTrekDoor: There's at lest one more Resp to play."
            )
            code = self.DoorStack[0]
    ...
    def UpdateDoorState(self, StateNum):
        if StateNum != self.DoorState:
            ...
            if self.DoorState == doorSDLstates["opening"]:
                ...
                PtDebugPrint(
                    "xHighLevelStarTrekDoor: Notifying Clients to play Open Door Responder"
                )

            elif self.DoorState == doorSDLstates["closing"]:
                self.SendNote("respCloseDoor;0")
                PtDebugPrint(
                    "xHighLevelStarTrekDoor: Notifying Clients to play Close Door Responder"
                )
    ...
    def ExecCode(self, code):
        try:
            ...
            if tag == "respOpenDoor":
                ...
                PtDebugPrint(
                    "xHighLevelStarTrekDoor.ExecCode(): ERROR! Invalid tag '%s'."
                    % (tag)
                )
                self.DoorStack.pop(0)
        except:
