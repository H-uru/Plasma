...
class xAgeSDLIntChange(ptResponder):
    ...
    def OnNotify(self, state, id, events):
        ...
        if not PtWasLocallyNotified(self.key):
            ...
            if actTrigger.value:
                PtDebugPrint(
                    "DEBUG: xAgeSDLIntChange.OnNotify():\t local player requesting %s change via %s"
                    % (stringVarName.value, actTrigger.value[0].getName())
                )
                pass
        ...
        PtDebugPrint(
            "DEBUG: xAgeSDLIntChange.OnNotify():\t%s age SDL var %s to %d"
            % (stringOp, stringVarName.value, intCurrentValue)
        )
    ...
    def OnSDLNotify(self, VARname, SDLname, playerID, tag):
        ...
        if VARname == stringVarName.value:
            PtDebugPrint(
                "DEBUG: xAgeSDLIntChange.OnSDLNotify():\t VARname:%s, SDLname:%s, tag:%s, value:%d"
