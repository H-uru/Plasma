...
clothingTypeList = [
    ...
    kFaceClothingItem,
    kShirtClothingItem,
    kRightHandClothingItem,
    kPantsClothingItem,
    kRightFootClothingItem,
]
...
guildSDLValues = {
    ...
    "MTorso_GuildRed": 3,
    "FTorso_GuildRed": 3,
    "MTorso_GuildYellow": 4,
    "FTorso_GuildYellow": 4,
    "MTorso_GuildWhite": 5,
    "FTorso_GuildWhite": 5,
}
...
class xTakableClothing(ptModifier):
    ...
    def OnFirstUpdate(self):
        ...
        if not stringVarName.value:
            PtDebugPrint(
                "ERROR: xTakableClothing.OnFirstUpdate():\tERROR: missing SDL var name on %s"
                % self.sceneobject.getName()
            )
        if not stringFClothingName.value:
            PtDebugPrint(
                "ERROR: xTakableClothing.OnFirstUpdate():\tERROR: missing female clothing name on %s"
                % self.sceneobject.getName()
            )
        if not stringMClothingName.value:
            PtDebugPrint(
                "ERROR: xTakableClothing.OnFirstUpdate():\tERROR: missing male clothing name on %s"
                % self.sceneobject.getName()
            )
        if not stringChanceSDLName.value:
            PtDebugPrint(
                "DEBUG: xTakableClothing.OnFirstUpdate(): Chance SDL var name is empty, so we will not use chance rolls for showing %s"
                % self.sceneobject.getName()
            )
        ...
        if AgeStartedIn == PtGetAgeName():
            ...
            if stringVarName.value:
                try:
                    ...
                    if not (ageSDL[stringVarName.value][0] ^ boolShowOnTrue.value):
                        PtAtTimeCallback(
                            self.key, 1, kEnableClothingTimer
                        )  # we will handle the clickables in a second, since handling them now doesn't work
                        self.shown = True
                    else:
                        PtAtTimeCallback(self.key, 1, kDisableClothingTimer)
                except:
                    PtDebugPrint(
                        "ERROR: xTakableClothing.OnServerInitComplete():\tERROR accessing ageSDL on %s so we're disabling"
                        % self.sceneobject.getName()
                    )
                    PtAtTimeCallback(self.key, 1, kDisableClothingTimer)
                    pass
                ...
                PtDebugPrint(
                    "ERROR: xTakableClothing.OnServerInitComplete():\tERROR: missing SDL var name on %s so we're disabling"
                    % self.sceneobject.getName()
                )
                PtAtTimeCallback(self.key, 1, kDisableClothingTimer)
                pass
            ...
            if self.useChance and not self.shown:  # only attempt to show if it's currently hidden
                try:
                    ...
                    PtDebugPrint(
                        "DEBUG: xTakableClothing.OnServerInitComplete(): Chance of %s appearing is %d percent"
                        % (self.sceneobject.getName(), self.chanceAppearing)
                    )
                    PtAtTimeCallback(
                        self.key, 1.5, kRollDiceTimer
                    )  # make sure this fires after our initial hide/show code runs
                except:
                    PtDebugPrint(
                        "ERROR: xTakableClothing.OnServerInitComplete():\tERROR missing SDL var %s so we will not use chance rolls on %s"
                        % (stringChanceSDLName.value, self.sceneobject.getName())
                    )
                    self.useChance = False
    ...
    def OnSDLNotify(self, VARname, SDLname, playerID, tag):
        ...
        try:
            ...
            PtDebugPrint(
                "ERROR: xTakableClothing.OnSDLNotify():\tERROR reading age SDL on %s so we're disabling"
                % self.sceneobject.getName()
            )
            self.IDisableClothing()
            pass
    ...
    def IGetHairColor(self):
        ...
        for item in worn:
            ...
            if type == kHairClothingItem:
                ...
                return color
        PtDebugPrint(
            "ERROR: xTakableClothing.IGetHairColor():  Couldn't find the currently worn hair item, defaulting to white"
        )
                            ...
                            if varnode:
                                if varnode.getType() == 4:
                                    ...
                                    if (
                                        ...
                                        or varName.find("Torso_GuildRed") != -1
                                        or varName.find("Torso_GuildYellow") != -1
                                        or varName.find("Torso_GuildWhite") != -1
                                    ):
                                        PtDebugPrint(
    ...
    def OnNotify(self, state, id, events):
        ...
        if id == actClickable.id:
            ...
            if not self.IItemInCloset():
                ...
                if boolHasHairColor.value:
                    PtDebugPrint(
                        "DEBUG: xTakableClothing.OnNotify():  Using existing hair color since this is a hair item"
                    )
                    color1 = hairColor
                ...
                if (
                    base.find("Torso_GuildBlue") != -1
                    or base.find("Torso_GuildGreen") != -1
                    or base.find("Torso_GuildRed") != -1
                    or base.find("Torso_GuildYellow") != -1
                    or base.find("Torso_GuildWhite") != -1
                ):
                ...
                PtDebugPrint(
                    "DEBUG: xTakableClothing.OnNotify():  You already have "
                    + base
                    + " so I'm not going to give it to you again"
                )
    ...
    def IEnableClothing(self):
        if self.IItemInCloset():
            ...
            if not "Guild" in stringFClothingName.value:
                PtDebugPrint(
                    "DEBUG: xTakableClothing.IEnableClothing():  Disabling clickable on %s because we already have it..."
                    % self.sceneobject.getName()
                )
