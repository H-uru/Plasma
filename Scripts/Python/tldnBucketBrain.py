...
kBucketStates = Enum(
    "Stop, QRun, QBoardQRun, Run, Dump, QStop, DumpQBoard, DumpQStop, DumpQBoardQStop, QBoard, Boarded"
)
...
class tldnBucketBrain(ptResponder):
    ...
    def OnServerInitComplete(self):
        ...
        if leverPulledState:
            ...
            PtDebugPrint(
                "tldnBucketBrain.OnServerInitComplete():\tLever starting in the pulled state (on)",
                level=kDebugDumpLevel,
            )
        else:
            PtDebugPrint(
                "tldnBucketBrain.OnServerInitComplete():\tLever starting in the pushed state (off)",
                level=kDebugDumpLevel,
            )
        ...
        if numPlayers == 0:
            ...
            PtDebugPrint(
                "tldnBucketBrain.OnServerInitComplete():\tRe-initializing age as I'm the only one here!",
                level=kDebugDumpLevel,
            )
            ...
            ageSDL[kStringAgeSDLBucketState] = (
                state,
                1,
            )  # use FF here because we'll run manually below...
            self.RunBucketState(
                state, 0
            )  # Start the buckets if necessary (we do it this just incase there was no update to the SDL)
    ...
    def AvatarPage(self, avaObj, pageIn, lastOut):
        ...
        for rider in riders:
            if rider == avaID:
                ...
                PtDebugPrint("Tye: Resetting avatar at entry!")
                PtDebugPrint(
                    "tldnBucketBrain.AvatarPage():\tbucket rider left age, pruning bucket rider list",
                    level=kDebugDumpLevel,
                )
                rider = -1
    ...
    def OnSDLNotify(self, VARname, SDLname, playerID, tag):
        ...
        if VARname == kStringAgeSDLBucketState:
            ...
            PtDebugPrint(
                "Debug: tldnBucketBrain.OnSDLNotify(): *****>Changed Bucket State to: %s, with FF = %s"
                % (kBucketStates.ToString(curBucketState), fastforward),
                level=kDebugDumpLevel,
            )
            ...
            # Set WCP bucket LED
            offStates = (
                kBucketStates.QBoard,
                kBucketStates.Stop,
                kBucketStates.QRun,
                kBucketStates.QBoardQRun,
                kBucketStates.Boarded,
            )
    ...
    def OnNotify(self, state, id, events):
        ...
        if id == rgnSensor01.id or id == rgnSensor02.id or id == rgnSensor03.id:
            for event in events:
                ...
                if event[1] == 1:  # a bucket entered rngBucketSnsEntryPoint
                    ...
                    return
            PtDebugPrint(
                "DEBUG: tldnBucketBrain.OnNotify():\t No local avatars in the bucket that triggered the camera change.",
                level=kDebugDumpLevel,
            )
            return
        ...
        if id == respWRCCGo.id:
            PtDebugPrint(
                "tldnBucketBrain.OnNotify():\tGot callback from workroom control panel bucket mode switch!  Trying to start buckets!",
                level=kDebugDumpLevel,
            )
            ...
            if ageSDL[kStringAgeSDLPowerOn][0] == 0:
                PtDebugPrint(
                    "tldnBucketBrain.OnNotify():\tCannot start the buckets from the workroom when the power is off!",
                    level=kDebugDumpLevel,
                )
                return
        ...
        if id == actTimerBktDelay.id:
            ...
            if not ageSDL[kStringAgeSDLLeverPulled][0]:
                PtDebugPrint(
                    "tldnBucketBrain.OnNotify():\tSomeone pushed the lever back, ignoring Run Timer CB.",
                    level=kDebugDumpLevel,
                )
                return
        ...
        if id == actTimerPwrOff.id:
            ...
            if ageSDL[kStringAgeSDLLeverPulled][0] and ageSDL[kStringAgeSDLPowerOn][0]:
                PtDebugPrint(
                    "tldnBucketBrain.OnNotify():\tPower is off or level is pulled, ignoring Stop Timer CB.",
                    level=kDebugDumpLevel,
                )
                return
        ...
        if id == actEntryPoint.id:
            ...
            for event in events:
                ...
                if event[1] == 0:  # a bucket exited rngBucketSnsEntryPoint
                    ...
                    if bucketAtEntry < 0:
                        PtDebugPrint(
                            "ERROR: tldnBucketBrain.OnNotify():\tCannot enable a bucket that is not at the entry point!",
                            level=kErrorLevel,
                        )
                    elif ageSDL[kStringAgeSDLRiders][bucketAtEntry] < 1:
                        PtDebugPrint(
                            "tldnBucketBrain.OnNotify():\tEncountered a bucket entering the loading point, the index is: ",
                            bucketAtEntry,
                            level=kDebugDumpLevel,
                        )
        ...
        if id == actBktEnter1.id or id == actBktEnter2.id:
            ...
            boardStates = [
                ...
                kBucketStates.QRun,
                kBucketStates.Dump,
                kBucketStates.DumpQStop,
            ]
            if not (bucketState in boardStates):
                PtDebugPrint(
                    "DEBUG: tldnBucketBrain.OnNotify():\tCannot enter bucket during an improper state: %s"
                    % kBucketStates.ToString(bucketState)
                )
                actBktEnter1.disable()
                actBktEnter2.disable()
                return
            ...
            if self.RiderInDockedBucket():
                PtDebugPrint(
                    "DEBUG: tldnBucketBrain.OnNotify()\tCannot enter bucket while it is occupied",
                    level=kDebugDumpLevel,
                )
                return
        ...
        if id == actBktExit.id:
            ...
            boardedStates = [
                ...
                kBucketStates.Dump,
                kBucketStates.QRun,
            ]  # Note: QRun requires a person to be in the bucket!
            if not (bucketState in boardedStates):
                ...
                if avaObj == None or bucketState != kBucketStates.QRun:
                    PtDebugPrint(
                        "DEBUG: tldnBucketBrain.OnNotify():\tCannot exit a bucket during an improper state: %s"
                        % kBucketStates.ToString(bucketState)
                    )
                    actBktExit.disable()
                    return
        ...
        if id == respBktExit.id:
            ...
            if bucketAtEntry < 0:
                PtDebugPrint(
                    "ERROR: tldnBucketBrain.OnNotify():\tAn avatar is trying to exit bucket #%s that is not at the entry point: "
                    % bucketAtEntry,
                    level=kErrorLevel,
                )
                return
        ...
        if (
            ...
            try:
                ...
                PtDebugPrint(
                    "ERROR: tldnBucketBrain.OnNotify():\trespBktOpen-->There is no avatar in the bucket to dump!!",
                    level=kErrorLevel,
                )
        ...
        if id == respDumpAvatar.id:
            if avaIDAtDump < 0:
                PtDebugPrint(
                    "ERROR: tldnBucketBrain.OnNotify():\trespDumpAvatar-->Avatar at dump point is less than zero!",
                    level=kErrorLevel,
                )
                return
    ...
    def OnControlKeyEvent(self, controlKey, activeFlag):
        ...
        if bucketAtEntry < 0 or avaID != ageSDL[kStringAgeSDLRiders][bucketAtEntry]:
            PtDebugPrint(
                "tldnBucketBrain.OnControlKeyEvent():\tIgnoring exit request as the bucket is not at the entry point",
                level=kDebugDumpLevel,
            )
            return
        ...
        if curBucketState != kBucketStates.Boarded:
            PtDebugPrint(
                "tldnBucketBrain.OnControlKeyEvent()\tCannot exit buckets while they are not in the Boarded State",
                level=kDebugDumpLevel,
            )
            return
        ...
        if (
            ...
            or controlKey == PlasmaControlKeys.kKeyMoveForward
        ):
            PtDisableControlKeyEvents(self.key)
    ...
    def UpdateBucketState(self, param, val=0):
        ...
        if error:
            ...
            PtDebugPrint(
                "tldnBucketBrain.UpdateBucketState(): ----[ Updating EVERYONE's State to: %s ]----"
    ...
    def EnterExitBucket(self):
        ...
        if avaID < 0:
            ...
            PtDebugPrint(
                "tldnBucketBrain.EnterExitBucket():\tAvatar Exiting bucket---Running AvatarExitBucket()!",
                level=kDebugDumpLevel,
            )
            ...
            try:
                ...
                PtDebugPrint(
                    "ERROR: tldnBucketBrain.EnterExitBucket():\tCouldn't find the avatar exiting the bucket!!!",
                    level=kErrorLevel,
                )
                return
        ...
        try:
            ...
            PtDebugPrint(
                "ERROR: tldnBucketBrain.EnterExitBucket():\tCouldn't find an avatar to add to the bucket!!!",
                level=kErrorLevel,
            )
            return
    ...
    def getAvatarFromBucketAtEntry(self):
        ...
        if bucketAtEntry < 0:
            PtDebugPrint(
                "ERROR: tldnBucketBrain.getAvatarFromBucketAtEntry():\tCannot retrieve the avatar from a bucket that's not at the entry point",
                level=kErrorLevel,
            )
            return None
        avaID = ageSDL[kStringAgeSDLRiders][bucketAtEntry]
        if avaID < 1:
            PtDebugPrint(
                "ERROR: tldnBucketBrain.getAvatarFromBucketAtEntry():\t Cannot retrieve the avatar from an empty bucket!",
                level=kErrorLevel,
            )
            return None
        avaKey = PtGetAvatarKeyFromClientID(avaID)
