...
class kdshGlowInTheDark(ptResponder):
    ...
    def OnServerInitComplete(self):
        ...
        if solved:
            ...
            PtDebugPrint(
                "\tThere are",
                len(PtGetPlayerList()),
                " other players in this Kadish instance.",
            )
            if len(PtGetPlayerList()) == 0:
                ...
                PtDebugPrint(
                    "\t Someone else was already in this Kadish instance. Depending on Overriding HighSDL component to synch elevator."
                )
        else:
            PtDebugPrint(
                "\t The puzzle has not been solved. Putting elevator at the top."
            )
    ...
    def OnNotify(self, state, id, events):
        ...
        if id == actSwitch01.id:
            ...
            if oldstate == 1:  # roof was previously closed
                ...
                PtDebugPrint(
                    "kdshGlowInTheDark: Unexpected roof state: (%s)" % newstate
                )
            ...
            if id == rgnExitSubTop.id:
                ...
                PtDebugPrint(
                    "You stepped on the elevator at the top. Joining subworld."
                )
                ...
                PtDebugPrint(
                    "You stepped on the elevator at the bottom. Joining subworld."
                )
        ...
        for event in events:
            if (
                event[0] == kVariableEvent
            ):  # Did another player just get out of the bucket by hitting ESC/backspace? (An OnControlKeyEvent I wouldn't have received)
                TimerID = int(event[3])
                # PtDebugPrint("\tTimer #", TimerID,"callback came through.")

                if TimerID == 1:
                    PtDebugPrint(
                        "kdshGlowInTheDark: Timer 1 Callback. Raising elevator again."
                    )
                ...
                if TimerID == 2:
                    PtDebugPrint(
                        "kdshGlowInTheDark: Timer 2 Callback. Clearing top Xrgn"
                    )
                ...
                if TimerID == 3:
                    ...
                    if solved:
                        PtDebugPrint(
                            "\t Puzzle is still solved. Lowering Elevator again."
                        )
                        ...
                        PtDebugPrint(
                            "\t Puzzle has been reset. Leaving elevator alone at top."
                        )
                        rgnEnterSubTop.disable()
                        for region in objExitTop.value:
                            region.physics.suppress(0)
                if TimerID == 4:
                    ...
                    PtDebugPrint(
                        "\tkdshGlowInTheDark.OnTimer: Running from Lit to Dark."
                    )
                    respFloorDark.run(self.key)
                    Resetting = 0
    ...
    def OnSDLNotify(self, VARname, SDLname, playerID, tag):
        ...
        if VARname == "RoofClosed":
            ...
            GlowCharged = ageSDL["GlowCharged"][0]
            PtDebugPrint(
                "\tkdshGlowInTheDark.OnSDLNotify: GlowCharged now =", GlowCharged
            )
    ...
    def BatonPassCheck(self, id, events):
        ...
        for event in events:
            if (
                event[0] == 7
            ):  # pruning a redundant event, so we only process enters and exits
                break
            ...
            if event[1] == 1:  # Player enters a zone
                ...
                if (
                    ...
                    id == baton + 6
                ):  # true if player is entering a new zone, and the "baton" is in one less zone. Progress is being made here in solving the puzzle.
                    baton = baton + 1

            elif event[1] == 0:  # Player exits a zone
                PtDebugPrint(" kdshGlowInTheDark: Exited Zone:", id - 5)
                if (
                    baton != (id - 4) and baton != 0
                ):  # Correctly advancing players always enter a new zone before exiting the current one. If this progress hasn't already been made, drop the baton.
                    PtDebugPrint("kdshGlowInTheDark: Dropped the baton.")
                    baton = 0

                if (
                    id == 14 and baton == 10
                ):  # if you just exited region 9, you are in region 10, and your baton is 10, you solved the puzzle
                    solved = ageSDL["GlowInTheDarkSolved"][0]
                    if not solved:
                        ...
                        PtDebugPrint(
                            "GlowInTheDark: Yes, you completed the path, but I thought the path was already solved."
                        )
                        return
    ...
    def OnTimer(self, id):
        ...
        if id == 5:
            PtDebugPrint(
                "kdshGlowInTheDark: It's been one second. FF'ing floor to glow state."
            )
            respFloorGlow.run(self.key, fastforward=1)
