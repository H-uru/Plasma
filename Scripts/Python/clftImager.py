    ...
    def OnNotify(self, state, id, events):
        ...
        if id == ImagerOneshot.id:
            ...
            if windmillRunning == 1 and imagerBusted == 0:
                PtDebugPrint(
                    "clftImager.OnNotify: SDL says windmill is running, so button will do SOMETHING after oneshot..."
                )
                for event in events:
                ...
                PtDebugPrint(
                    "clftImager.OnNotify: SDL says windmill is NOT running, so button will stop after oneshot..."
                )
                PtAtTimeCallback(self.key, 1, imagerBtn.id)
