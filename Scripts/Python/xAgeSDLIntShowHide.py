...
class xAgeSDLIntShowHide(ptMultiModifier):
    ...
    def Initialize(self):
        ...
        if stringVarName.value:
            ...
            try:
                ...
                PtDebugPrint(
                    "ERROR: xAgeSDLIntShowHide.OnServerInitComplete():\tERROR: couldn't process start state list"
                )
                pass
            ...
            try:
                if SDLvalue in self.enabledStateList:
                    PtDebugPrint(
                        "DEBUG: xAgeSDLIntShowHide.OnServerInitComplete: Attempting to enable drawing and collision on %s..."
                    ...
                    PtDebugPrint(
                        "DEBUG: xAgeSDLIntShowHide.OnServerInitComplete: Attempting to disable drawing and collision on %s..."
                        % self.sceneobject.getName()
                    )
                ...
                PtDebugPrint(
                    "ERROR: xAgeSDLIntShowHide.OnServerInitComplete():\tERROR enabling/disabling object %s"
                    % self.sceneobject.getName()
                )
                self.runDefault()
        else:
    ...
    def EnableObject(self):
        PtDebugPrint(
            "DEBUG: xAgeSDLIntShowHide.EnableObject:  Attempting to enable drawing and collision on %s..."
            % self.sceneobject.getName()
        )
        self.sceneobject.draw.enable()
        self.sceneobject.physics.suppress(False)
    ...
    def DisableObject(self):
        PtDebugPrint(
            "DEBUG: xAgeSDLIntShowHide.DisableObject:  Attempting to disable drawing and collision on %s..."
            % self.sceneobject.getName()
        )
        self.sceneobject.draw.disable()
        self.sceneobject.physics.suppress(True)
