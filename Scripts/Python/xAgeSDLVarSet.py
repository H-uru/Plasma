...
class xAgeSDLVarSet(ptResponder):
    ...
    def OnServerInitComplete(self):
        ...
        try:
            ...
            PtDebugPrint(
                "ERROR: xAgeSDLVarSet.OnServerInitComplete():\tERROR: age sdl read failed, SDLvalue = %d by default. stringSDLVarName = %s"
    ...
    def OnSDLNotify(self, VARname, SDLname, PlayerID, tag):
        ...
        if int(SDLvalue) in self.enabledStateDict:
            ...
            if stringSDLVarToSet:
                ageSDL.setTagString(stringSDLVarToSet.value, stringTag.value)
            PtDebugPrint(
                "DEBUG: xAgeSDLVarSet.OnServerInitComplete:\t%s setting %s to %d, tag string: %s"
                % (
                    stringSDLVarName.value,
                    stringSDLVarToSet.value,
                    self.enabledStateDict[int(SDLvalue)],
