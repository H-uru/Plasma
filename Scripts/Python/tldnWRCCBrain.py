...
class tldnWRCCBrain(ptResponder):
    ...
    def Load(self):
        ...
        if boolOperated:
            if solo:
                PtDebugPrint(
                    "tldnWRCCBrain.Load():\tboolOperated=%d but no one else here...correcting"
                    % boolOperated
                )
                boolOperated = 0
                self.SDL["boolOperated"] = (0,)
                ...
                PtDebugPrint(
                    "tldnWRCCBrain.Load():\tboolOperated=%d, disabling WRCC clickable"
                    % boolOperated
                )
    ...
    def OnServerInitComplete(self):
        ...
        if AgeStartedIn == PtGetAgeName():
            ...
            try:
                ...
                hatchLocked = True
                PtDebugPrint(
                    "tldnWRCCBrain.OnServerInitComplete():\tERROR: age sdl read failed, defaulting:"
                )
            PtDebugPrint(
                "tldnWRCCBrain.OnServerInitComplete():\t%s = %d, %s=%d"
                % (kStringAgeSDLPowerOn, pwrOn, kStringAgeSDLCabinDrained, cabinDrained)
            )
            PtDebugPrint(
                "tldnWRCCBrain.OnServerInitComplete():\t%s = %d, %s=%d"
            ...
            if cabinDrained:
                respDrainLvr.run(
                    self.key, state="drain", fastforward=True
                )  # move the needle to drained position
            ...
            try:
                ...
                PtDebugPrint(
                    "tldnWRCCBrain.OnServerInitComplete():\tCould not read Bucket Loop Mode SDL, defaulting to: ON"
                )
                bucketMode = True
    ...
    def OnSDLNotify(self, VARname, SDLname, playerID, tag):
        ...
        if AgeStartedIn == PtGetAgeName():
            ageSDL = PtGetAgeSDL()
            PtDebugPrint(
                "tldnWRCCBrain.OnSDLNotify():\t VARname:%s, SDLname:%s, tag:%s, value:%d, playerID:%d"
    ...
    def OnNotify(self, state, id, events):
        ...
        for event in events:  # this detects the untrigger of the sitting component
            if event[0] == 6 and event[1] == 1 and id == actSitComponent.id:
                if state == 0:  # true as player stands up
                    PtDebugPrint(
                        "tldnWRCCBrain.OnNotify():\tWRCC unoccupied, re-enabling WRCC sit clickable."
                    )
                    actSitClickable.enable()
                    self.SDL["boolOperated"] = (0,)
                    self.SDL["OperatorID"] = (-1,)
                    ...
                    if PtWasLocallyNotified(self.key):
                        ...
                        PtDebugPrint(
                            "tldnWRCCBrain.OnNotify():\tI'm the one who stood up, will hide panel's clickables"
                        )
        ...
        if id == actSitClickable.id:
            ...
            if PtWasLocallyNotified(self.key):
                ...
                PtDebugPrint(
                    "tldnWRCCBrain.OnNotify():\tI'm the one who sat down, will show panel's clickables"
                )
                respWRCCMaster.run(self.key, state="off")
        ...
        if id == actDrainLvr.id:
            if AgeStartedIn == PtGetAgeName():
                ...
                if pwrOn and hatchLocked and not cabinDrained:
                    ...
                    ageSDL.setTagString(
                        kStringAgeSDLCabinDrained, "ignore"
                    )  # ignore so we don't set global cabinDrained yet...see respCabinDrain callback below
                    ageSDL[kStringAgeSDLCabinDrained] = (
                        1,
                    )  # set this now so folks coming in will just see it drained
                    ...
                    PtDebugPrint(
                        "tldnWRCCBrain.OnNotify:\tCan't operate drain -- pwrOn=%d hatchLocked=%d."
                        % (pwrOn, hatchLocked)
                    )
            return
        ...
        if id == actLockSwitch.id:
            if AgeStartedIn == PtGetAgeName():
                ...
                if pwrOn and not hatchOpen and not cabinDraining:
                    ...
                    PtDebugPrint(
                        "tldnWRCCBrain.OnNotify:\tCan't unlock hatch -- pwrOn=%d hatchOpen=%d cabinDraining=%d."
                        % (pwrOn, hatchOpen, cabinDraining)
                    )
                else:
                    respLockSwitch.run(self.key, state="lockfail")
                    PtDebugPrint(
                        "tldnWRCCBrain.OnNotify:\tCan't lock hatch -- pwrOn=%d hatchOpen=%d cabinDraining=%d."
                        % (pwrOn, hatchOpen, cabinDraining)
                    )
            return
        ...
        if id == actOpenHatchAbv.id:
            if AgeStartedIn == PtGetAgeName():
                ...
                if hatchLocked:
                    ...
                    respHatchGlare.run(
                        self.key, events=events, state="on"
                    )  # panel glare on - hatch is open
            return
        ...
        if id == actCloseHatch.id:
            if AgeStartedIn == PtGetAgeName():
                ...
                respHatchGlare.run(
                    self.key, events=events, state="off"
                )  # panel glare off - hatch is closed
            return
