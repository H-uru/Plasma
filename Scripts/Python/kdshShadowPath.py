...
actSwitch01 = ptAttribActivator(
    1, "Actvr: Switch 01"
)  # Switches are numbered left to right in a counterclockwise fashion
...
respSwitch01 = ptAttribResponder(
    6, "Rspndr: Switch 01", ["on", "off"]
)  # responders which animate the motion of the switches themselves
...
class kdshShadowPath(ptResponder):
    ...
    def Load(self):
        ...
        while count < 5:
            PtDebugPrint(
                "kdshShadowPath.Load(): ageSDL[ShadowPathLight0",
                count,
                "]=%d" % (ageSDL["ShadowPathLight0" + str(count)][0]),
            )
            count = count + 1
