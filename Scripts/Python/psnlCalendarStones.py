        ...
        if fire:
            if ageSDL:
                try:
                    ...
                    PtDebugPrint(
                        "psnlCalendarStones.OnServerInitComplete():\tERROR reading age SDLVar for YeeshaPage20. Assuming value = 0"
                    )
                    CurrentValue = 0
                if CurrentValue in [0, 2, 4]:
                    PtDebugPrint(
                        "psnlCalendarStones.OnServerInitComplete():  don't have YeeshaPage20 on, no FIRE for you!"
                    )
                    respCalStoneFire.run(self.key, state="off", fastforward=1)
                else:
                    PtDebugPrint(
                        "psnlCalendarStones.OnServerInitComplete():  have all 12 calendar stones AND YeeshaPage20 is on, will give you FIRE!"
                    )
                    fireworks = 1
            ...
            PtDebugPrint(
                "psnlCalendarStones.OnServerInitComplete():  don't have all 12 calendar stones, no FIRE for you!"
            )
            respCalStoneFire.run(self.key, state="off", fastforward=1)
