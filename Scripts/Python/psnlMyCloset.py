        ...
        if AgeStartedIn == PtGetAgeName():
            ...
            if vault.amOwnerOfCurrentAge():
                ...
                try:
                    ...
                    PtDebugPrint(
                        "psnlCloset.OnServerInitComplete():\tERROR reading SDL from vault, defaulting to closed (fastforward)"
                    )
                    self.ICloseCloset(1)
                if not closetClosed:
                ...
                PtAtTimeCallback(
                    self.key, 1, kVisitorDisableTimer
                )  # we will disable the clickables in a second, since disabling them now doesn't work
                try:
                    closetClosed = ageSDL[kSDLClosetClosed][0]
                except:
                    PtDebugPrint(
    ...
    def OnSDLNotify(self, VARname, SDLname, playerID, tag):
        ...
        if VARname == kSDLClosetClosed:
            if AgeStartedIn == PtGetAgeName():
                ageSDL = PtGetAgeSDL()
                PtDebugPrint(
                    "psnlClosetDoor.OnSDLNotify():\t VARname:%s, SDLname:%s, tag:%s, value:%d, playerID:%d"
