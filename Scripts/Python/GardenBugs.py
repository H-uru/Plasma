    ...
    def OnTimer(self, id):
        ...
        if id == kAddBugs:
            ...
            if particlesToTransfer < 0:
                ...
                PtDebugPrint(
                    "GardenBugs.OnTimer() - Particles to kill: "
                    + str(particlesToKill)
                    + " ("
                    + str(percentToKill * 100)
                    + "%)"
                )
    ...
    def OnNotify(self, state, id, events):
        ...
        if id == bugRgn.id:
            ...
            if PtLocalAvatarRunKeyDown() and PtLocalAvatarIsMoving():
                PtDebugPrint(
                    "gardenBugs.OnNotify()-->\tcan't add bugs as we're still running (but our running flag is False?)"
                )
                return
    ...
    def OnBehaviorNotify(self, behavior, id, state):
        ...
        if not state:
            ...
            if behavior == PtBehaviorTypes.kBehaviorTypeRun:
                ...
                if PtLocalAvatarRunKeyDown() and PtLocalAvatarIsMoving():
                    PtDebugPrint(
                        "gardenBugs.OnBehaviorNotify()-->\tWARN: Running behavior turned off, but avatar still reports run and movement keys are held down"
                    )
            return
        else:
        ...
        if self.bugCount > 0:
            if (
                behavior == PtBehaviorTypes.kBehaviorTypeStandingJump
                or behavior == PtBehaviorTypes.kBehaviorTypeWalkingJump
            ):
                if numJumps == 0:
                    numJumps = 1
                    if self.bugCount == 1:
            ...
            if (
                behavior == PtBehaviorTypes.kBehaviorTypeRunningImpact
                or behavior == PtBehaviorTypes.kBehaviorTypeRunningJump
            ):
