...
respUrwinSfx = ptAttribResponder(
    21,
    "resp: Urwin SFX",
    [
        "Eat2Idle",
    ...
    def OnNotify(self, state, id, events):
        ...
        if id == (-1):
            ...
            if id == respUrwinSfx.id:
                ...
                if (
                    id == respUrwin_Walk_Loop01.id
                    or id == respUrwin_Walk_Loop02.id
                    or id == respUrwin_WalkSniff_ToWalk.id
                    or id == respUrwin_Idle_ToWalk.id
                ):
                    ...
                    id == respUrwin_Walk_ToWalkSniff.id
                    or id == respUrwin_WalkSniff.id
                    or id == respUrwin_Eat_ToWalkSniff.id
                ):
                    UrwinMasterAnim.animation.resume()
                    ...
                    id == respUrwin_WalkSniff_ToEat.id or id == respUrwin_Idle_ToEat.id
                ):
                    ...
                    id == respUrwin_Idle_01.id
                    or id == respUrwin_Idle_02.id
                    or id == respUrwin_Eat_ToIdle.id
                    or id == respUrwin_Walk_ToIdle.id
                    or id == respUrwin_Idle_Vocalize.id
                ):
                    UrwinMasterAnim.animation.stop()
                    pct = random.randint(0, 4)
                    if pct == 4:
    ...
    def InitNewSDLVars(self):
        ...
        while isinstance(spawnTimes[-1], int):
            ...
            if newTime < endOfToday:
                PtDebugPrint(
                    "payiUrwinBrain: Generated a valid spawn time: %d" % (newTime)
                )
        ...
        if ageSDL["UrwinSpawnTimes"][0]:
            for timer in ageSDL["UrwinSpawnTimes"]:
                if timer:
                    ...
                    if timeTillSpawn > 0:
                        PtDebugPrint(
                            "payiUrwinBrain: Setting timer for %d seconds"
                            % (timeTillSpawn)
                        )
            ...
            PtDebugPrint(
                "payiUrwinBrain: Setting EndOfDay timer for %d seconds"
                % (timeLeftToday)
            )
            ...
            if code.find("respUrwin") != -1:
                ...
                if ecState == "Idle":
                    if ecAction == "01":
                        ...
                        PtDebugPrint(
                            "payiUrwinBrain.ExecCode(): ERROR! Invalid ecAction '%s'."
                            % (ecAction)
                        )
                        stackList.pop(0)
                elif ecState == "Walk":
                    ...
                    PtDebugPrint(
                        "payiUrwinBrain.ExecCode(): ERROR! Invalid ecState '%s'."
                        % (ecState)
                    )
                    stackList.pop(0)
            else:
                PtDebugPrint(
                    "payiUrwinBrain.ExecCode(): ERROR! Invalid code '%s'." % (code)
                )
                stackList.pop(0)
        except:
            PtDebugPrint(
