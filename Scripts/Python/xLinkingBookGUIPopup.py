...
class xLinkingBookGUIPopup(ptModifier):
    ...
    def OnNotify(self, state, id, events):
        ...
        if id == shareRegion.id:
            if PtWasLocallyNotified(self.key):
                for event in events:
                    if (
                        event[0] == kCollisionEvent and not event[1]
                    ):  # False being an exit
                        PtDebugPrint(
                            "xLinkingBookGUIPopup: exited book offer region",
        ...
        # am I walking here as a result of accepting the offered book
            ...
            PtDebugPrint(
                "xLinkingBookGUIPopup: notified of share book seek beh",
                level=kDebugDumpLevel,
            )
            for event in events:
                PtDebugPrint(
                    "xLinkingBookGUIPopup: event[0]=", event[0], level=kDebugDumpLevel
                )
                PtDebugPrint(
                    "xLinkingBookGUIPopup: event[1]=", event[1], level=kDebugDumpLevel
                )
                PtDebugPrint(
                    "xLinkingBookGUIPopup: event[2]=", event[2], level=kDebugDumpLevel
                )
                ...
                for event in events:
                    if event[0] == kVariableEvent:
                        ...
                        try:
                            ...
                            PtDebugPrint(
                                "xLinkingBookGUIPopup: Somebody lost my bookmark. Assuming CurrentPage = 1",
                                level=kErrorLevel,
                            )
                            CurrentPage = 1
                        idRequestor = event[3]
                        PtDebugPrint(
                            "xLinkingBookGUI.OnNotify():\tpsnlBookshelf user id %d selected book %s from the shelf"
                            % (idRequestor, stringAgeRequested),
                            level=kDebugDumpLevel,
                        )
        ...
        # is it a clickable book on a pedestal?
            ...
            if PtWasLocallyNotified(self.key) and state:
                ...
                if (
                    ...
                    PtDebugPrint(
                        "xLinkingBookGUIPopup: Smart seek used", level=kDebugDumpLevel
                    )
        ...
        # is it the seek behavior because we clicked on a book ourself?
            ...
            for event in events:
                if (
                    ...
                    reportingClient = PtGetClientIDFromAvatarKey(
                        reportingAvatar.getKey()
                    )
        ...
        # else was it one of the unknown id types? like scene input interface or from a ptBook?
            ...
            for event in events:
                ...
                if (
                    event[0] == kOfferLinkingBook
                ):  # this can come locally (to re-draw the panel after someone has accepted or rejected it) or...
                    PtDebugPrint(
                        "xLinkingBookGUIPopup: got offer book notification",
                        level=kDebugDumpLevel,
                    )
                    ...
                    if messageAv != localAv:
                        ...
                        return
                    PtDebugPrint(
                        "xLinkingBookGUIPopup: offered book message for me",
                        level=kDebugDumpLevel,
                    )
                    if (
                        event[2] == -999
                        ...
                        BookOfferer = event[1]
                        PtDebugPrint(
                            "xLinkingBookGUIPopup: offered book by %s"
                            % (BookOfferer.getName()),
                            level=kDebugDumpLevel,
                        )
                        avID = PtGetClientIDFromAvatarKey(BookOfferer.getKey())
                ...
                # is it from the OpenBook? (we only have one book to worry about)
                    ...
                    PtDebugPrint(
                        "xLinkingBookGUIPopup: BookNotify  event=%d, id=%d"
                        % (event[1], event[2])
                    )
                    if event[1] == PtBookEventTypes.kNotifyImageLink:
                        if event[2] == xLinkingBookDefs.kShareBookLinkID:
                            ...
                            PtDebugPrint(
                                "xLinkingBookGUIPopup:Book: hit share panel; age filename = %s, age instancename = %s"
                                % (self.IGetAgeFilename(), self.IGetAgeInstanceName())
                            )
                            PtSetOfferBookMode(
                                self.key,
                                self.IGetAgeFilename(),
                                self.IGetAgeInstanceName(),
                            )
                            PtSetShareSpawnPoint(self.IGetAgeSpawnPoint())
                            if self.IGetAgeFilename() == "PelletBahroCave":
                                PtDebugPrint(
                                    "using Adam's plasma-magical 'PtSetShareAgeInstanceGuid' with pelletcaveguid = ",
                                    pelletCaveGUID,
                                )
                            ...
                            PtDebugPrint(
                                "xLinkingBookGUIPopup:Book: hit linking panel %s"
                                % (event[2])
                            )
                            ...
                            if OfferedBookMode and BookOfferer:
                                ...
                                if len(actBookshelf.value) == 0:  # Pedestal Book
                                    ...
                                    if ptVault().amOwnerOfCurrentAge():
                                        ...
                                        PtDebugPrint(
                                            "xLinkingBookGUIPopup: Placing a bookmark for book #%d on page %d"
                                            % (BookNumber, CurrentPage),
                                            level=kDebugDumpLevel,
                                        )
                                        ageSDL = PtGetAgeSDL()
                                        ageSDL.setIndex(
                                            "CurrentPage", BookNumber, CurrentPage
                                        )
                                    ...
                                    note.addVarNumber(
                                        "ILink"
                                        + ","
                                        + SpawnPointName_Dict[event[2]]
                                        + ","
                                        + SpawnPointTitle_Dict[event[2]],
                                        -1.0,
                                    )
                                    note.netPropagate(0)
                                    note.send()
                        ...
                        PtDebugPrint(
                            "xLinkingBookGUIPopup:Book: NotifyShow",
                            level=kDebugDumpLevel,
                        )
                        # re-allow KI and BB
                        ...
                        if CurrentPage > 1:
                            PtDebugPrint(
                                "xLinkingBookGUIPopup: going to page %d (ptBook page %d)"
                        ...
                        PtDebugPrint(
                            "xLinkingBookGUIPopup:Book: NotifyHide",
                            level=kDebugDumpLevel,
                        )
                        ...
                        if not ClosedBookToShare:
                            ...
                            if OfferedBookMode and BookOfferer:
                                ...
                                PtNotifyOffererLinkRejected(avID)
                                PtDebugPrint(
                                    "xLinkingBookGUIPopup: rejected link, notifying offerer as such",
                                    level=kDebugDumpLevel,
                                )
                                OfferedBookMode = False
                                BookOfferer = None
                        ...
                        if ForceThirdPerson.value:
                            ...
                            PtAtTimeCallback(
                                self.key, kFirstPersonEnableTime, kFirstPersonEnable
                            )
                        ...
                        PtDebugPrint(
                            "xLinkingBookGUIPopup:Book: NotifyCheckUncheck",
                            level=kDebugDumpLevel,
                        )
                        pass
        ...
        if agePanel:
            ...
            if agePanel == "Cleft":
                ...
                if entry is not None:
                    ...
                    entry.save()
                    PtDebugPrint(
                        "Chronicle entry TomahnaLoad already added, setting to yes"
                    )
                else:
                    sdl = xPsnlVaultSDL()
                    sdl["CleftVisited"] = (1,)
            ...
            try:
                ...
                PtDebugPrint(
                    "xLinkingBookGUIPopup: could not find age %s's linking panel"
                    % (agePanel),
                    level=kErrorLevel,
                )
        else:
            PtDebugPrint(
                "xLinkingBookGUIPopup: no age link panel" % (agePanel),
                level=kErrorLevel,
            )
    ...
    def IShowBookTreasure(self):
        ...
        if agePanel:
            ...
            if (
                ...
                SpawnPointTitle_Dict = {
                    xLinkingBookDefs.kFirstLinkPanelID: "DisabledDesert"
                }
                SpawnPointName_Dict = {
                    xLinkingBookDefs.kFirstLinkPanelID: "DisabledDesert"
                }
            ...
            try:
                ...
                if len(params) == 6:
                    ...
                    if not fromBookshelf:
                        ...
                        sharable, width, height, stampdef, gui = (
                            0,
                            1.0,
                            1.0,
                            xLinkingBookDefs.NoDRCStamp,
                            "BkBook",
                        )
                        bookdef = (
                            xLinkingBookDefs.BookStart1
                            + xLinkingBookDefs.DRCStampHolder
                            + xLinkingBookDefs.NoShare
                            + xLinkingBookDefs.LinkStart
                            + params
                            + xLinkingBookDefs.LinkEnd
                        )
                ...
                if "NotPossible" in SpawnPointTitle_Dict.values():
                    bookdef = (
                        ...
                        + xLinkingBookDefs.LinkStart
                        + "xLinkPanelBlackVoid"
                        + xLinkingBookDefs.LinkEndNoLink
                    )
                    sharable = 0
            ...
            if (
                fromBookshelf
                and xLinkingBookDefs.kBookMarkID in SpawnPointTitle_Dict.keys()
            ):
                if SpawnPointTitle_Dict[xLinkingBookDefs.kBookMarkID] == "JCSavePoint":
                    bookmark = xLinkingBookDefs.JCBookMark
                elif (
                    SpawnPointTitle_Dict[xLinkingBookDefs.kBookMarkID] == "SCSavePoint"
                ):
            ...
            if sharable:
                ...
                if agePanel == "CleftWithTomahna" and ptVault().amOwnerOfCurrentAge():
                    allPagesDef = bookdef % (
                        bookmark,
                        stampdef,
                        xLinkingBookDefs.kFirstLinkPanelID,
                        xLinkingBookDefs.kFirstLinkPanelID + 1,
                    )
                else:
            ...
            if agePanel != "CleftWithTomahna":
                for linkID in SpawnPointTitle_Dict.keys():
                    if (
                        ...
                        continue
                    try:
                        pagedef = xLinkingBookDefs.xLinkingPages[
                            SpawnPointTitle_Dict[linkID]
                        ]
                        ...
                        try:
                            ...
                            PtDebugPrint(
                                "xLinkingBookGUIPopup: Treasure page %s's book definition doesn't look like a page???"
                                % (SpawnPointTitle_Dict[linkID]),
                                level=kErrorLevel,
                            )
                    except LookupError:
                        PtDebugPrint(
                            "xLinkingBookGUIPopup: could not find treasure book page %s's linking panel"
                            % (SpawnPointTitle_Dict[linkID]),
                            level=kErrorLevel,
                        )
    ...
    def IDoCityLinksChron(self, agePanel):
        ...
        if entryCityLinks is not None:
            ...
            if agePanel not in CityLinks:
                ...
                PtDebugPrint(
                    "xLinkingBookGUIPopup.IDoCityLinksChron():  do nothing, citylinks chron already contains: ",
                    agePanel,
                )
        else:
    ...
    def IGetCityLinksChron(self):
        ...
        if entryCityLinks is not None:
            valCityLinks = entryCityLinks.chronicleGetValue()
            PtDebugPrint(
                "xLinkingBookGUIPopup.IGetCityLinksChron(): valCityLinks = ",
                valCityLinks,
            )
            CityLinks = valCityLinks.split(",")
        return CityLinks
    ...
    def IShowBahroBook(self, linkingPanel):
        ...
        if PtIsSinglePlayerMode() or OfferedBookMode:
            bookdef = (
                "<font size=10>"
                + xLinkingBookDefs.BahroNoShare
                + "<pb>"
                + xLinkingBookDefs.TransLinkStart
                + linkingPanel
                + xLinkingBookDefs.LinkEnd
            )
        else:
    ...
    def BuildTreasureLinks(self, ageRequested):
                    ...
                    for ageDataChildRef in ageDataChildren:
                        ...
                        if chron and chron.getName() == "AhnonaySpawnPoints":
                            ...
                            for spawn in spawns:
                                ...
                                spawnPointInfo = ptSpawnPointInfo(
                                    spawnInfo[0], spawnInfo[1]
                                )
        ...
        SpawnPointTitle_Dict = {}
        PtDebugPrint(
            "xLinkingBookGUI.BuildTreasureLinks():The %s book has the following %s pages: "
            % (ageRequested, len(spawnPoints))
        )
        ...
        for spawnPoint in spawnPoints:
            ...
            if spawnPoint.getTitle() == "Default":
                ...
                spawnPoint.getTitle() == "JCSavePoint"
                or spawnPoint.getTitle() == "SCSavePoint"
            ):
                if len(actBookshelf.value) > 0:
                    SpawnPointTitle_Dict[
                        xLinkingBookDefs.kBookMarkID
                    ] = spawnPoint.getTitle()
                    SpawnPointName_Dict[
                        xLinkingBookDefs.kBookMarkID
                    ] = spawnPoint.getName()
                ...
                if HasFoundOriginalBook:
                    page = (
                        len(SpawnPointTitle_Dict) + xLinkingBookDefs.kFirstLinkPanelID
                    )
                else:
                    page = (
                        len(SpawnPointTitle_Dict)
                        + xLinkingBookDefs.kFirstLinkPanelID
                        + 1
                    )

                PtDebugPrint(
                    "\tPage #%s: spawnpoint: %s, LinkPanel/Title: %s"
                    % (page, spawnPoint.getName(), spawnPoint.getTitle())
                )
        ...
        if not HasFoundOriginalBook:
            if ageRequested == "Neighborhood":
                PtDebugPrint(
                    "\tPage #1: You didn't find the original book, but you're looking at the neighborhood. The first panel shows %s"
                    % (ageRequested)
                )
                # goes in the front of the list
                SpawnPointName_Dict[
