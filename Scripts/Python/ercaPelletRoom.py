...
class ercaPelletRoom(ptResponder):
    ...
    def OnServerInitComplete(self):
        ...
        if entry is not None:
            ...
            if oldGotPellet != 0:
                ...
                entry.save()
                PtDebugPrint(
                    "ercaPelletRoom.OnServerInitComplete(): chron entry GotPellet still contained a recipe, setting to 0"
                )
    ...
    def OnNotify(self, state, id, events):
        ...
        if id == ActUseMachine.id and state and LocalAvatar == PtFindAvatar(events):
            ...
            if TakePellet:
                PtDebugPrint(
                    "TakePellet is still active, must wait a few seconds before allowing another pellet to be spit out"
                )
            ...
            if byteChamber == 5:
                PtDebugPrint(
                    "Trying to spit pellet, but we're on Chamber5.  This shouldn't be possible.  Closing machine..."
                )
                ageSDL[SDLMachine.value] = (0,)
                ageSDL[SDLChamber.value] = (0,)
                ageSDL[SDLPellet1.value] = (0,)
