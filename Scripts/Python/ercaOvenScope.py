...
class ercaOvenScope(ptModifier):
    ...
    def OnServerInitComplete(self):
        ...
        PtDebugPrint(
            ...
            % (SDLBakeryPwr.value, ageSDL[SDLBakeryPwr.value][0])
        )
        try:
    ...
    def IDoTimerWheel(self):
        ...
        if CurTime < FinishTime:
            ...
            if StartTime < CurTime:
                PtDebugPrint(
                    "ercaOvenScope:IDoTimerWheel:  Now updating timer wheel of scope# %d to %d seconds remaining"
                    % (ScopeNum.value, TimeRemaining)
                )
