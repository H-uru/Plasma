...
class tldn3FloorElevator(ptResponder):
    ...
    def OnServerInitComplete(self):
        ...
        PtDebugPrint(
            "tldn3FloorElevator.OnServerInitComplete():\t%s=%d, %s=%d"
            % (kStringAgeSDLElvCurrFloor, elevCurrFloor, kStringAgeSDLElvIdle, elevIdle)
        )
    ...
    def OnSDLNotify(self, VARname, SDLname, playerID, tag):
        ...
        PtDebugPrint(
            "tldn3FloorElevator.OnSDLNotify():\t VARname:%s, SDLname:%s, tag:%s, value:%d, playerID:%d"
    ...
    def OnNotify(self, state, id, events):
        ...
        if elevLocked:
            if (
                (id == actCall1.id and elevCurrFloor == 1)
                or (id == actCall2.id and elevCurrFloor == 2)
                or (id == actCall3.id and elevCurrFloor == 3)
            ):
                return
            else:
                PtDebugPrint(
