...
class ercaBakeryElev(ptResponder):
    ...
    def OnServerInitComplete(self):
        ...
        PtDebugPrint(
            ...
            % (SDLPlatform1.value, ageSDL[SDLPlatform1.value][0])
        )
        try:
        ...
        PtDebugPrint(
            "DEBUG: ercaBakeryElev.OnServerInitComplete():\t%s = %d"
            % (SDLPlatform2.value, ageSDL[SDLPlatform2.value][0])
        )
        try:
        ...
        PtDebugPrint(
            "DEBUG: ercaBakeryElev.OnServerInitComplete():\t%s = %d"
            % (SDLPlatform3.value, ageSDL[SDLPlatform3.value][0])
        )
        try:
        ...
        PtDebugPrint(
            "DEBUG: ercaBakeryElev.OnServerInitComplete():\t%s = %d"
            % (SDLPlatform4.value, ageSDL[SDLPlatform4.value][0])
        )
        try:
    ...
    def OnSDLNotify(self, VARname, SDLname, playerID, tag):
        ...
        ageSDL = PtGetAgeSDL()
        PtDebugPrint(
            "ercaBakeryElev.OnSDLNotify():\t VARname: %s, SDLname: %s, tag: %s, value: %d, playerID: %d"
