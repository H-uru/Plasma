...
class ercaControlRoom(ptResponder):
    ...
    def OnServerInitComplete(self):
        ...
        PtDebugPrint(
            ...
            % (SDLImgrView.value, ageSDL[SDLImgrView.value][0])
        )
        try:
        ...
        if id == ActScrollLeft.id and state and LocalAvatar == PtFindAvatar(events):
            ...
            if OnInit:
                ...
                if byteImgrNew == 1:
                    if byteMixBtnNew != 0:
                        ...
                        PtDebugPrint(
                            "DEBUG: ercaControlRoom.OnServerInitComplete():\tOn Mixer view, but btn SDL is 0, so no mixer icons."
                        )
                        RespMixIcons.run(self.key, state="off", fastforward=0)
                else:
                    PtDebugPrint(
                        "DEBUG: ercaControlRoom.OnServerInitComplete():\tNot on mixer view, so no mixer icons."
                    )
                    RespMixIcons.run(self.key, state="off", fastforward=0)
                ...
                if byteImgrNew == 0:
                    if byteOvenBtnNew != 0:
                        ...
                        PtDebugPrint(
                            "DEBUG: ercaControlRoom.OnServerInitComplete():\tOn Oven view, but btn SDL is 0, so no oven icons."
                        )
            ...
            if byteMixBtnNew != 0:
                ...
                if ageSDL[blade][0] == 0:
                    ...
                    if ageSDL[hatch][0] == 1:
                        ageSDL[hatch] = (0,)
                        PtDebugPrint(
                            "Blades starting up, but hatch is unlocked.  Will now lock hatch, setting SDL for %s to %d"
                            % (hatch, 1)
                        )
                else:
                    ageSDL[blade] = (0,)
            ...
            if byteOvenBtnNew != 0:
                ...
                if (ageSDL["ercaBakeryElevPos"][0] != 0) or byteAmBaking:
                    PtDebugPrint(
                        "Tried to operate an oven-scope, but bakery elevator is in use or am baking; request denied!"
                    )
                    return
                if ageSDL[oven][0] == 0:
                    ageSDL[oven] = (1,)
