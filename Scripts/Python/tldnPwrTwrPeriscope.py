...
class tldnPwrTwrPeriscope(ptResponder):
    ...
    def OnServerInitComplete(self):
        ...
        if AgeStartedIn == PtGetAgeName():
            ...
            try:
                ...
                if pumpCount == 3:
                    boolTwrRaised = True
                PtDebugPrint(
                    "tldnPwrTwrPeriscope.OnServerInitComplete():\tageSDL[%s] = %d, TwrRaised = %d"
                    % (kStringAgeSDLPumpCount, pumpCount, boolTwrRaised)
                )
            except:
                PtDebugPrint(
                    "tldnPwrTwrPeriscope.OnServerInitComplete():\tERROR: age sdl read failed, defaulting tower lowered"
                )
            if boolTwrRaised:
                actSun.enable()
            else:
            ...
            try:
                ...
                PtDebugPrint(
                    "tldnPwrTwrPeriscope.OnServerInitComplete():\tPwrMain=%d"
                    % (boolPwrMain)
                )
            except:
                PtDebugPrint(
            ...
            if boolPwrMain and not boolTwrRaised:
                PtDebugPrint(
                    "tldnPwrTwrPeriscope.OnServerInitComplete():\tturning off main power because tower not raised"
                )
        ...
        PtDebugPrint(
            "tldnPwrTwrPeriscope.Load():\tscope values: left=%f,up=%f,top=%d,btm=%d"
            % (scopeSpdLeft, scopeSpdUp, boolScopeAtTop, boolScopeAtBtm)
        )
    ...
    def AvatarPage(self, avObj, pageIn, lastOut):
        ...
        PtDebugPrint(
            "tldnPwrTwrPeriscope.AvatarPage(): Client ID %d paging out" % (avID)
        )
        PtDebugPrint(
    ...
    def OnSDLNotify(self, VARname, SDLname, playerID, tag):
        ...
        if AgeStartedIn == PtGetAgeName():
            ageSDL = PtGetAgeSDL()
            PtDebugPrint(
                "tldnPwrTwrPeriscope.OnSDLNotify():\t VARname:%s, SDLname:%s, tag:%s, value:%d, playerID:%d"
            ...
            if VARname == kStringAgeSDLPumpCount:
                ...
                if boolTwrRaised:
                    ...
                    if boolScopeOperator:
                        ...
                        GUIDialog.refreshAllControls()
                    if (
                        boolPwrMain and not boolTwrRaised
                    ):  # power on, tower being lowered
                        PtDebugPrint(
            ...
            if VARname == kStringAgeSDLMainPowerOn:
                ...
                if not boolPwrMain:
                    if tag == "towerLowered":
                        PtAtTimeCallback(
                            self.key, 5, kTimerIDDelayPowerOff
                        )  # delay power off till tower actually coming down
                        actSun.disable()
                        # animScopeLeft.value.stop()
                        # animScopeUp.value.stop()
                        # scopeSpdUp = 0
                ...
                if boolPwrMain:
                    ...
                    ageSDL[kStringAgeSDLPumpCount] = (
                        3,
                    )  # raise the tower...if it's already raised this val will already be 3 and nothing will come back to us, otherwise force the tower to raise
                    respPowerOn.run(self.key)
                return
    ...
    def OnNotify(self, state, id, events):
                ...
                for event in events:
                    if event[0] == kCollisionEvent:
                        ...
                        if linedUp:  # sun now lined up with scope
                            PtDebugPrint(
                                "tldnPwrTwrPeriscope.OnNotify():\tturning on main power because dish is lined up"
                            )
                            ageSDL.setTagString(
                                kStringAgeSDLMainPowerOn, "towerAligned"
                            )
                            ageSDL[kStringAgeSDLMainPowerOn] = (1,)
                        else:
                            PtDebugPrint(
                                "tldnPwrTwrPeriscope.OnNotify():\tturning off main power because dish is not lined up"
                            )
                            ageSDL.setTagString(
                                kStringAgeSDLMainPowerOn, "towerMisaligned"
                            )
                            ageSDL[kStringAgeSDLMainPowerOn] = (0,)
                        break
            return
        ...
        if state and id == Activate.id:
            ...
            if PtWasLocallyNotified(self.key):
                ...
                PtDebugPrint(
                    "tldnPwrTwrPeriscope.OnNotify:\twrote SDL - scope operator id = %d"
                    % (avID)
                )
                PtDebugPrint(
                    "tldnPwrTwrPeriscope.OnNotify:\tSDL OperatorID = %d"
                    % (self.SDL["OperatorID"])
                )
                GUIDialog = PtGetDialogFromString(Vignette.value)
                GUICbxBlocker = ptGUIControlCheckBox(
    ...
    def IEngageTelescope(self):
        ...
        PtDebugPrint(
            "tldnPwrTwrPeriscope.IEngageTelescope():\tentering telescope...left=%f,up=%f,top=%d,btm=%d"
            % (scopeSpdLeft, scopeSpdUp, boolScopeAtTop, boolScopeAtBtm)
        )
    ...
    def IQuitTelescope(self):
        ...
        PtAtTimeCallback(
            self.key, 3, kTimerIDClearOperator
        )  # Put in this delay to avoid people entering the scope before they've fully exited it
