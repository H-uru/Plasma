...
class xButtonLeverSwitchPowered(ptResponder):
    ...
    def OnNotify(self, state, id, events):
        ...
        if state:
                ...
                for event in events:
                    if event[0] == 4:
                        if event[3] == 1:  # power on
                            ...
                            PtDebugPrint(
                                "xButtonLeverSwitchPowered.OnNotify:\t'%s' ERROR---got bogus msg - power = %d"
                                % (varstring.value, self.SDL["enabled"][0])
                            )
                            return
                        PtDebugPrint(
                            "xButtonLeverSwitchPowered.OnNotify:\t'%s' got msg - power = %d"
                            % (varstring.value, self.SDL["enabled"][0])
                        )
            ...
            if id == respOn.id:
                PtDebugPrint(
                    "xButtonLeverSwitchPowered.OnNotify:\tsending msg '%s' clicked, pulled or otherwise activated."
                    % varstring.value
                )
