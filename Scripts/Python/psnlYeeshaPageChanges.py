...
class psnlYeeshaPageChanges(ptMultiModifier):
            ...
            if self.ageSDL:
                                ...
                                if ageSDL is not None:
                                    ...
                                    if (
                                        ...
                                        if not val:
                                            HideCleftPole = 1
                                            PtDebugPrint(
                                                "psnlYeeshaPageChanges.OnServerInitComplete():\t Fissure is open, so setting HideCleftPole = ",
                                                HideCleftPole,
                                            )
                                else:
                                    PtDebugPrint(
                                        "ERROR: psnlYeeshaPageChanges.OnServerInitComplete():\tProblem trying to access age SDLs for Bahro poles"
                                    )
                                    pass

                            except:
                                PtDebugPrint(
                                    "ERROR: psnlYeeshaPageChanges.OnServerInitComplete():\tException occurred trying to access age SDL"
                                )
                ...
                if PageNumber.value == 5 and stringShowStates.value == "0":
                    ...
                    for thispage in range(1, TotalPossibleYeeshaPages + 1):
                        ...
                        PtDebugPrint(
                            "\t The previous value of the SDL variable %s is %s"
                ...
                PtDebugPrint(
                    "psnlYeeshaPageChanges: Error trying to access the Chronicle self.ageSDL. self.ageSDL = %s"
                    % (self.ageSDL)
                )
        else:
            PtDebugPrint(
                "psnlYeeshaPageChanges: Error trying to access the Vault. Can't access YeeshaPageChanges chronicle."
            )
    ...
    def EnableDisable(self, val):
        if val in self.enabledStateList:
            if PageNumber.value == 10:
                PtDebugPrint(
                    "psnlYeeshaPageChanges: Attempting to enable drawing and collision on %s..."
                ...
                if HideCleftPole:
                    PtDebugPrint(
                        "psnlYeeshaPageChanges.EnableDisable():\tFissure is open and Totem yeesha page is set 'off', so we gotta get rid of the Cleft pole for now"
                    )
            ...
            if PageNumber.value == 10:
                PtDebugPrint(
                    "psnlYeeshaPageChanges: Attempting to disable drawing and collision on %s..."
                    % self.sceneobject.getName()
                )
            self.sceneobject.draw.disable()
            self.sceneobject.physics.suppress(True)
    ...
    def UpdateState(self, state, size, SDLVar, AgeVault, sizechanged):
        ...
        if state == 3:
            state = 2
            PtDebugPrint(
                "psnlYeeshaPageChanges: Updated value of YeeshaPage %s from 3 to 2."
            ...
            state = 1
            PtDebugPrint(
                "psnlYeeshaPageChanges: Updated value of YeeshaPage %s from 4 to 1."
                % ("YeeshaPage" + str(PageNumber.value))
            )
            SDLVar.setInt((size * 10) + state)
            AgeVault.updateAgeSDL(self.ageSDL)
