...
def SaveAvatarToDisk():
    ...
    for i in range(len(clothingList)):
        ...
        item += (
            ...
            + str(color1[i].getRed())
            + " "
            + str(color1[i].getGreen())
            + " "
            + str(color1[i].getBlue())
        )
        item += (
            " "
            + str(color2[i].getRed())
            + " "
            + str(color2[i].getGreen())
            + " "
            + str(color2[i].getBlue())
        )
        saveFile.write(item + "\n")
    saveFile.write(
        str(skinColor.getRed())
        + " "
        + str(skinColor.getGreen())
        + " "
        + str(skinColor.getBlue())
        + "\n"
    )
        ...
        if (
            ...
            if (
                ...
                if CanShowSeasonal(clothingItem):
                    ...
                    PtDebugPrint(
                        "CanShowClothingItem(): Hiding item "
                        + clothingItem.name
                ...
                PtDebugPrint(
                    "CanShowClothingItem(): Hiding item "
                    + clothingItem.name
                    + " because it is a multiplayer-only option"
                )
        else:
            PtDebugPrint(
                "CanShowClothingItem(): Hiding item "
                + clothingItem.name
                + " because it is optional and isn't in your closet"
            )
    else:
        PtDebugPrint(
            "CanShowClothingItem(): Hiding item "
            + clothingItem.name
            + " because it is an internal-only option"
        )
    return False
...
def IsRightArrow(id):
    if (id >= kIDBtnHairRightAccArrow and id <= kIDBtnFeetRightAccArrow) or (
        id >= kIDBtnHairRightOptArrow and id <= kIDBtnFeetRightOptArrow
    ):
        return 1
    return 0
...
def IsLeftArrow(id):
    if (id >= kIDBtnHairLeftAccArrow and id <= kIDBtnFeetLeftAccArrow) or (
        id >= kIDBtnHairLeftOptArrow and id <= kIDBtnFeetLeftOptArrow
    ):
        return 1
    return 0
...
def IsAccArrow(id):
    if (id >= kIDBtnHairLeftAccArrow and id <= kIDBtnFeetLeftAccArrow) or (
        id >= kIDBtnHairRightAccArrow and id <= kIDBtnFeetRightAccArrow
    ):
        return 1
    return 0
...
def IsOptArrow(id):
    if (id >= kIDBtnHairLeftOptArrow and id <= kIDBtnFeetLeftOptArrow) or (
        id >= kIDBtnHairRightOptArrow and id <= kIDBtnFeetRightOptArrow
    ):
        return 1
    return 0
...
class xAvatarCustomization(ptModifier):
                ...
                # is it one of the list boxes, probably one of the clothing listboxes
                    ...
                    if tagID == kUpperBodyOptionsLB or tagID == kLwrBodyOptionsLB:
                        ...
                        if itemselect == -1:
                            ...
                            listboxDict[tagID + kAccessoryLBOffset].SelectItem(
                                0
                            )  # select the first texture option
                    ...
                    # else it might be one of the accessory listboxes
                        ...
                        if (
                            tagID - kAccessoryLBOffset == kUpperBodyOptionsLB
                            or tagID - kAccessoryLBOffset == kLwrBodyOptionsLB
                        ):
                            ...
                            if itemselect == -1:
                                ...
                                if not newitem.donotwear:
                                    if newitem.coloredAsHair:
                                        ...
                                        if tagID == kHeadAccLB and not (
                                            newitem.name in untintableHeadAcc
                                        ):
                                            self.IShowColorPicker(kColor2ClickMap)
                                            colorbar2 = ptGUIControlTextBox(
                                                AvCustGUI.dialog.getControlFromTag(
                                            ...
                                            if not lastitem == "":
                                                lastcolor = avatar.avatar.getTintClothingItem(
                                                    lastitem, 1
                                                )
                                            else:
                    ...
                    if tagID == kPanelsRGID:
                        ...
                        zoomBtn = ptGUIControlCheckBox(
                            AvCustGUI.dialog.getControlFromTag(kZoomButton)
                        )
                        if rgVal == 1:
                            ...
                            ZoomCamera.sceneobject.pushCutsceneCamera(
                    ...
                    if btnID == kLinkBackBtnID:
                        ...
                        if not ptNetLinkingMgr().isEnabled():
                            PtDebugPrint(
                                "OnGuiNotify():\tAborting linkout attempt because the linking managar isn't ready"
                            )
        ...
        for item in WornList:
            if item.seasonal and not ItemInWardrobe(item):
                ...
                avatar.avatar.addWardrobeClothingItem(
                    item.name, ptColor().white(), ptColor().white()
                )
    ...
    def OnTimer(self, id):
        ...
        if id == 999:
            ...
            if not PtCreateDir(basePath):
                PtDebugPrint(
                    'xAvatarCustomization::OnTimer(): Unable to create "'
                    + basePath
                    + '" directory. Avatar pic is NOT saved.'
                )
                return
    ...
    def IUpdateClothingListboxes(self, avatar):
        ...
        for listboxID in TheCloset.keys():
            ...
            if not (listboxID + kAccessoryLBOffset in listboxDict):
                ...
                listboxDict[listboxID + kAccessoryLBOffset].SetListboxID(
                    listboxID + kAccessoryLBOffset
                )
    ...
    def ISetStandardControls(self):
        ...
        if self.dirty:  # has the clothing changed at all?
            ptGUIControlButton(
                AvCustGUI.dialog.getControlFromTag(kAvatarResetID)
            ).show()  # then show the reset button
        else:
            ptGUIControlButton(
                AvCustGUI.dialog.getControlFromTag(kAvatarResetID)
        ...
        for morphID in range(kNumberOfMorphs):
            ...
            try:
                ...
                PtDebugPrint(
                    "Some error occurred while setting morph slider #"
                    + str(morphID)
                    + ", morphs probably haven't loaded yet"
                )
        ...
        for texMorphID in range(1, kNumberOfTexMorphs + 1):
            ...
            try:
                ...
                PtDebugPrint(
                    "Some error occurred while setting tex morph slider #"
                    + str(texMorphID)
...
class ClothingItem:
    def __init__(
        ...
        self.seasonTime = (
            []
        )  # the date that it will show up on (0 for any of these means any, i.e. 0/1/03 means the first day of every month in 2003
                ...
                for part in parts:
                    ...
                    if ls == "colorlabel1":
                        ...
                        if rs == "yes":
                            ...
                            PtDebugPrint(
                                "AvaCusta: Unknown inCloset type of %s on clothing item %s"
                                % (rs, self.name)
                            )
                        ...
                        if rs == "yes":
                            ...
                            PtDebugPrint(
                                "AvaCusta: Unknown inClosetColor1 type of %s on clothing item %s"
                                % (rs, self.name)
                            )
                        ...
                        if rs == "yes":
                            ...
                            PtDebugPrint(
                                "AvaCusta: Unknown inClosetColor2 type of %s on clothing item %s"
                                % (rs, self.name)
                            )
                    elif ls == "nonstandard":
                        ...
                        try:
                            ...
                            for dateRange in ranges:
                                ...
                                if len(dates) == 1:
                                    ...
                                    if (startDate[2] == 0) or (endDate[2] == 0):
                                        startDate[2] = endDate[
                                            2
                                        ] = 0  # make sure that if one year is wild, the other is also
                                    ...
                                    while not (startDate == endDate):
                                        ...
                                        if startDate[1] >= 31:  # end of the month
                                            ...
                                            if startDate[0] >= 12:  # end of the year
                                                startDate[
                                                    0
                                                ] = 1  # reset to Jan 1st of the next year
                                                startDate[1] = 1
                                                startDate[2] = startDate[2] + 1
                                            else:
                                                startDate[0] = (
                                                    startDate[0] + 1
                                                )  # reset to the first of the next month
                                                startDate[1] = 1
                                        else:
                                            if (
                                                startDate[0] == endDate[0]
                                                and startDate[2] == endDate[2]
                                            ):  # we have the right month and year, so set to the right day
                            ...
                            PtDebugPrint(
                                "AvaCusta: Malformed date string "
                                + rs
                                + " on clothing "
                                + self.name
                            )
                        ...
                        if ls != "":
                            PtDebugPrint(
                                "AvaCusta: Unknown keyword type (%s) on clothing %s"
                                % (ls, self.name)
                            )
...
class ClothingCloset:
    ...
    def getItemByName(self, itemName):
        ...
        texGroup = TextureGroup(
            kShirtClothingItem, itemName
        )  # clothing type doesn't matter since we don't use it here
...
class ScrollingListBox:
    ...
    def SelectItem(self, listboxIndex):
        ...
        if listboxIndex >= 0 and listboxIndex < 4:
            ...
            if (
                ...
                selection = (
                    listboxIndex + self.offset - 4 + self.columns
                )  # add the offset, subtract 4, then add columns to get the right value
    ...
    def SetClothingList(self, clothingList):
        ...
        if self.rows == 1:
            ...
            self.columns = int(
                round(float(len(self.clothingList)) / 2.0)
            )  # half of the number of items, rounded up
        self.offset = 0
        self.selection = 0
    ...
    def UpdateListbox(self):
        ...
        if (
            self.listboxID == kUpperBodyAccLB or self.listboxID == kLwrBodyAccLB
        ):  # we don't display anything if only one texture option is available
        ...
        if self.rows == 1 and len(self.clothingList) <= 4:
            ...
            for i in range(4):
                displayItems.append(
                    self.clothingList[i + self.offset]
                )  # add the first row of items to display
            if self.rows == 2:
                for i in range(4):
                    try:
                        displayItems.append(
                            self.clothingList[i + self.offset + self.columns]
                        )  # add the second row
                    except:
        ...
        LBIndex = -1
        if (self.rows == 1 and len(self.clothingList) <= 4) or (
            self.rows == 2 and len(self.clothingList) <= 8
        ):
            ...
            if (
                self.selection >= self.offset and self.selection <= self.offset + 3
            ):  # on the top row
                LBIndex = self.selection - self.offset
            elif (
                self.rows == 2
                and self.selection >= self.offset + self.columns
                and self.selection <= self.offset + self.columns + 3
            ):  # on the bottom row
