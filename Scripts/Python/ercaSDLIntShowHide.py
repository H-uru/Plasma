...
class ercaSDLIntShowHide(ptMultiModifier):
        ...
        if AgeStartedIn == PtGetAgeName():
            ...
            if stringVarName.value:
                ...
                try:
                    ...
                    PtDebugPrint(
                        "ERROR: ercaSDLIntShowHide.OnServerInitComplete():\tERROR: age sdl read failed, SDLvalue = 0 by default. stringVarName = %s"
                        % (stringVarName.value)
                    )
                    SDLvalue = 0
                ...
                try:
                    if SDLvalue in self.enabledStateList:
                        PtDebugPrint(
                            "DEBUG: ercaSDLIntShowHide.OnServerInitComplete: Attempting to disable drawing and collision on %s..."
                        ...
                        PtDebugPrint(
                            "DEBUG: ercaSDLIntShowHide.OnServerInitComplete: Attempting to enable drawing and collision on %s..."
                            % self.sceneobject.getName()
                        )
                    ...
                    PtDebugPrint(
                        "ERROR: ercaSDLIntShowHide.OnServerInitComplete():\tERROR enabling/disabling object %s"
                        % self.sceneobject.getName()
                    )
                    pass
            else:
    ...
    def EnableObject(self):
        PtDebugPrint(
            "DEBUG: ercaSDLIntShowHide.EnableObject:  Attempting to enable drawing and collision on %s..."
            % self.sceneobject.getName()
        )
        self.sceneobject.draw.enable()
        self.sceneobject.physics.suppress(False)
    ...
    def DisableObject(self):
        PtDebugPrint(
            "DEBUG: ercaSDLIntShowHide.DisableObject:  Attempting to disable drawing and collision on %s..."
            % self.sceneobject.getName()
        )
        self.sceneobject.draw.disable()
        self.sceneobject.physics.suppress(True)
