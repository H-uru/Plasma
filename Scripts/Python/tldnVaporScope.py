...
class tldnVaporScope(ptModifier):
    ...
    def AvatarPage(self, avObj, pageIn, lastOut):
        ...
        if avID == self.SDL["OperatorID"][0]:
            ...
            self.SDL["boolOperated"] = (0,)
            PtDebugPrint(
                "tldnVaporScope.AvatarPage(): telescope operator paged out, reenabled telescope.",
                level=kDebugDumpLevel,
            )
        else:
            return
    ...
    def OnGUINotify(self, id, control, event):
        ...
        if event == kDialogLoaded:
            ...
            if control.isEnabled():
                ...
                PtDebugPrint(
                    "tldnVaporScope:ShowDialog:  current FOVh at %f - setting slider to %f"
                    % (gInitialFOV, zsliderValue)
                )
                            ...
                            if Vignette.value:
                                ...
                                if scopeDlg:
                                    try:
                                        ...
                                        PtDebugPrint(
                                            "tldnVaporScope:GUINotify can't find the fire button",
                                            level=kDebugDumpLevel,
                                        )
                        except KeyError:
                            PtDebugPrint(
    ...
    def IQuitTelescope(self):
        ...
        PtAtTimeCallback(
            self.key, kTimerDisengageTime, kTimerDisengage
        )  # wait for player to finish exit one-shot, then reenable clickable
