...
ImagerPOTSObject = ptAttribSceneobject(11, "Imager Object (for pots clue)")
ImagerPOTSTime = ptAttribInt(12, "Number of seconds until POTS Image Shows",default=870)
#----------
# globals
#----------
ImagerContents = []
...
class grtpImager(ptModifier):
    ...
    def __init__(self):
        ...
        self.id = 196
        self.version = 3
        PtDebugPrint("grtpImager: init  version=%d.%d"%(self.version,2),level=kWarningLevel)

    def vaultOperationStarted(self,context):
        PtDebugPrint("grtpImager: vaultOperationStarted(%s)"%(context),level=kDebugDumpLevel)

    def vaultOperationComplete(self,context,args,resultCode):
        PtDebugPrint("grtpImager: vaultOperationComplete(%s,%s)"%(context,resultCode),level=kDebugDumpLevel)
        if context==kAddingDevice:
            ...
            if resultCode>=0:
                ...
                if node:
                    ...
                    PtDebugPrint("grtpImager:ERROR! device node not found",level=kErrorLevel)
            ...
            if resultCode>=0:
                ...
                PtAtTimeCallback(Instance.key,ImagerPOTSTime.value,5)
...
###==== Cheat
        ...
        ImagerPOTSObject.sceneobject.draw.disable()
        ImagerPOTSObject.sceneobject.physics.suppress(True)
        ImagerObject.sceneobject.draw.enable()
        ImagerObject.sceneobject.physics.suppress(False)
        if ImagerMap.textmap is None:
            PtDebugPrint("grtpImager:ERROR! simpleImager[%s]: Dynamic textmap is broken!" % (ImagerName.value),level=kErrorLevel)
            return
        if ImagerObject.sceneobject is None:
            PtDebugPrint("grtpImager:ERROR! simpleImager[%s]: ImagerObject not specified!" % (ImagerName.value),level=kErrorLevel)
            return
        ...
        if ImagerName.value:
            ...
            PtDebugPrint("grtpImager: There was no imager name!",level=kErrorLevel)
        ...
        if AgeStartedIn == PtGetAgeName():
            if ImagerInboxVariable.value:
                ...
                try:
                    ...
                    PtDebugPrint("grtpImager: Problem reading the inbox from SDL")
    ...
    def IDetermineCensorLevel(self):
        ...
        PtDebugPrint("grtpImager: the censor level is %d" % (theCensorLevel),level=kWarningLevel)
    ...
    def OnTimer(self,id):
        ...
        if id == 5:
            ImagerPOTSObject.sceneobject.draw.enable()
            ImagerPOTSObject.sceneobject.physics.suppress(False)
            ImagerObject.sceneobject.draw.disable()
            ImagerObject.sceneobject.physics.suppress(True)
            PtAtTimeCallback(self.key,ImagerTime.value,6)
        elif id == 6:
            ImagerPOTSObject.sceneobject.draw.disable()
            ImagerPOTSObject.sceneobject.physics.suppress(True)
            ImagerObject.sceneobject.draw.enable()
            ...
            PtAtTimeCallback(Instance.key,ImagerPOTSTime.value,5)
        elif id == kFlipImagesTimerCurrent:
    ...
    def OnVaultEvent(self,event,tupdata):
        ...
        #~ PtDebugPrint("grtpImager[%s]:OnAgeKIEvent recvd. Event=%d and data= " % (ImagerName.value,event),tupdata)
        PtDebugPrint("grtpImager.OnVaultEvent[", ImagerName.value, "]: event = ", event, " tupdata = ", tupdata, level=kDebugDumpLevel)
        
        # Sometimes, we randomly get None... O.o
        if tupdata[0] == None:
            PtDebugPrint("grtpImager.OnVaultEvent: nil ptVaultNode", level=kErrorLevel)
            return None
        ...
        if isinstance(tupdata[0], ptVaultNode):
            ...
            PtDebugPrint("grtpImager.OnVaultEvent: tupdata[0] has unhandled object type", level=kWarningLevel)
            return None
        
        if not (type == PtVaultNodeTypes.kImageNode or type == PtVaultNodeTypes.kTextNoteNode):
            PtDebugPrint("grtpImager.OnVaultEvent: ... but we don't care!", level=kDebugDumpLevel)
            return None
        
        # make sure that the bigKI dialog is loaded before trying to update it
        if event == PtVaultCallbackTypes.kVaultNodeSaved:
            PtDebugPrint("grtpImager: kVaultNodeSaved event (id=%d,type=%d)" % (tupdata[0].getID(),tupdata[0].getType()), level=kDebugDumpLevel)
            ...
            PtDebugPrint("grtpImager: kVaultNodeRefAdded event (childID=%d,parentID=%d)" % (tupdata[0].getChildID(),tupdata[0].getParentID()),level=kDebugDumpLevel)
            # tupdata is ( ptVaultNodeRef )
            self.IRefreshImagerFolder()
            self.IRefreshImagerContent(tupdata[0])
        elif event == PtVaultCallbackTypes.kVaultRemovingNodeRef:
            #~ PtDebugPrint("grtpImager: kVaultRemovingNodeRef event (childID=%d,parentID=%d)" % (tupdata[0].getChildID(),tupdata[0].getParentID()))
            # tupdata is ( ptVaultNodeRef )
            self.IRefreshImagerFolder()
    ...
    def OnNotify(self,state,id,events):
        ...
        #~ PtDebugPrint("grtpImager: Notify event state=%f,id=%d,events=" % (state,id),events)
                    ...
                    if id == ImagerRegion.id:
                        for event in events:
                            if event[0] == kCollisionEvent:
                                ...
                                if event[1]:
                                    PtDebugPrint("grtpImager: add imager %s" % (ImagerName.value),level=kDebugDumpLevel)
                                    ...
                                    PtDebugPrint("grtpImager: remove imager %s" % (ImagerName.value),level=kDebugDumpLevel)
        ...
        if folder is not None:
            ...
            if ImagerObject.sceneobject.isLocallyOwned():
                if len(ImagerContents) > ImagerMax.value:
                    ...
                    PtDebugPrint("grtpImager[%s]: removing element %d" % (ImagerName.value,relem.getID()),level=kDebugDumpLevel)
                    ...
                    if element is not None and element.getID() == CurrentDisplayedElementID:
                        ...
                        if elemType == PtVaultNodeTypes.kImageNode:
                            ...
                            if textbody == "cleardaImager":
                                PtDebugPrint("grtpImager[%s]: clearing the imager of images" % (ImagerName.value),level=kWarningLevel)
                            ...
                            PtDebugPrint("grtpImager[%s]: Can't display element type %d" % (ImagerName.value,elemType),level=kWarningLevel)
                            pass
                        return
            else:
                PtDebugPrint("grtpImager[%s]: Inbox for imager is None" % (ImagerName.value),level=kWarningLevel)
        else:
            # undisplaying all images... to be done later...
            ImagerMap.textmap.clearToColor(ptColor(0,0,0,0))
            ImagerMap.textmap.flush()
            PtDebugPrint("grtpImager[%s]: no current element id to display" % (ImagerName.value),level=kDebugDumpLevel)
