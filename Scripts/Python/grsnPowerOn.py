...
switchGlowResponder = ptAttribResponder(
    37, "main switch glow indicator", ["On", "Off"], netForce=1
)
gearGlowResponder = ptAttribResponder(
    38, "gear switch glow indicator", ["On", "Off"], netForce=1
)
upElevGlowResponder = ptAttribResponder(
    39, "up switch glow indicator", ["On", "Off"], netForce=1
)
dnElevGlowResponder = ptAttribResponder(
    40, "dn switch glow indicator", ["On", "Off"], netForce=1
)

weightControlResponder = ptAttribResponder(
    41,
    "weight control responder",
    ["High", "Medium", "Low", "Lowest", "HighDown"],
    netForce=1,
)
weightTrigger = ptAttribActivator(42, "weight activator")

mainPowerOnResponder = ptAttribResponder(43, "main power on", netForce=1)
mainPowerOffResponder = ptAttribResponder(
    44, "main power off", ["GearOff", "GearDown"], netForce=1
)
mainBreakerResponder = ptAttribResponder(45, "main breaker responder", netForce=1)
gearRoomDoors = ptAttribResponder(
    46, "gear room doors", ["TurnOn", "TurnOff"], netForce=1
)
...
weightStartSoundResp = ptAttribResponder(
    52, "weight start sound responder", ["Start", "Loop", "Stop", "StopAll"], netForce=1
)
    ...
    def OnServerInitComplete(self):
        ...
        if AgeStartedIn == PtGetAgeName():
            ...
            if gearOn:
                ...
                mainPowerOnResponder.run(
                    self.key, avatar=PtGetLocalAvatar(), fastforward=True
                )
    ...
    def OnNotify(self, state, id, events):
        ...
        if id == weightTrigger.id:
            ...
            if not generatorPrimed:
                ...
                flashingLightsResponder.run(
                    self.key, state="Blink", avatar=PtGetLocalAvatar()
                )
        ...
        if id == weightStartSoundTrigger.id:
            if weightDirection == kUp:
                ...
                if weightSoundLoopingDown:
                    ...
                    weightSoundLoopingDown = False
                    weightStartSoundResp.run(
                        self.key, state="StopAll", avatar=triggerer
                    )
                else:
        ...
        if id == weightHighUp.id:
            ...
            if gearOn:
                return
            flashingLightsResponder.run(
                self.key, state="Off", avatar=PtGetLocalAvatar()
            )
            if mainOn:
                ageSDL[mainSwitchSDL.value] = (0,)
        ...
        if id == gearStopTrigger.id:
            ...
            if gearStopping:
                ...
                if gearStartingDown:
                    PtDebugPrint(
                        "gear stop trigger: gear still in initial descent - do nothing"
                    )
                else:
                    mainPowerOffResponder.run(
                        self.key, state="GearDown", avatar=PtGetLocalAvatar()
                    )

        if id == mainPowerOffResponder.id:
            if gearStartingDown:
                PtDebugPrint(
                    "main power responder callback - gear finished initial descent"
                )
            ...
            if not gearStopping and not gearStartingDown:
                ...
                if not generatorPrimed:
                    if not brake01On:
                        gearBrake01Resp.run(
                            self.key, state="AutoReturn", avatar=triggerer
                        )
                        ageSDL[gearBrake01SDL.value] = (1,)
        ...
        if id == mainSwitchResp.id:
            ...
            if mainOn:
                ...
                if gearOn:
                    ...
                    switchGlowResponder.run(self.key, state="Off", avatar=triggerer)
                    mainPowerOffResponder.run(
                        self.key, state="GearOff", avatar=triggerer
                    )
                    gearStopping = True
        ...
        if id == gearSwitchResp.id:
            ...
            if gearOn:
                ...
                if mainOn:
                    ...
                    ageSDL[mainSwitchSDL.value] = (0,)
                    mainSwitchResp.run(
                        self.key, state="PrimerExpired", avatar=triggerer
                    )
            ...
            if mainOn:
                if gearOn:
                    ...
                    if generatorPrimed and not brake01On and not brake02On:
                        ...
                        gearSwitchResp.run(
                            self.key, state="TripEngaged", avatar=triggerer
                        )
            else:  # main off
            ...
            if mainOn:
                if gearOn:
                    ...
                    if elevOn:
                        ageSDL[dnElevSwitchSDL.value] = (0,)
                        dnElevSwitchResp.run(
                            self.key, state="TurnOff", avatar=triggerer
                        )
                    else:
