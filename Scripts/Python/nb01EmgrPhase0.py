...
def Ayhoheek5Man1State(VARname, NewSDLValue):
    PtDebugPrint(
        "nb01EmgrPhase0.Ayhoheek5Man1State(): Attempting to set '{}' to a value of {}".format(
            VARname, NewSDLValue
        )
    )
    ...
    if NewSDLValue > nb01Ayhoheek5Man1StateMaxINT:
        PtDebugPrint(
            "ERROR: nb01EmgrPhase0.Ayhoheek5Man1State:\tERROR: Variable '{}'' expected range from  0-{}. Received value of {}.".format(
                VARname, nb01Ayhoheek5Man1StateMaxINT, NewSDLValue
            )
        )
        return
        ...
        PtDebugPrint(
            "DEBUG: nb01EmgrPhase0.Ayhoheek5Man1State:\tPaging out 5 Man Heek table completely."
        )
        ...
        PtDebugPrint(
            "DEBUG: nb01EmgrPhase0.Ayhoheek5Man1State:\tPaging in broken 5 Man Heek table."
        )
        ...
        PtDebugPrint(
            "DEBUG: nb01EmgrPhase0.Ayhoheek5Man1State:\tPaging in functional 5 Man Heek table."
        )
        ...
        PtDebugPrint(
            "ERROR: nb01EmgrPhase0.Ayhoheek5Man1State:\tERROR: Invalid value ({}) for variable '{}'.".format(
                NewSDLValue, VARname
            )
        )
        return
...
class nb01EmgrPhase0(ptResponder):
    ...
    Pedestals = {
        ...
        "Right": {
            ...
            "Books": {1: {"Name": "GreatZero", "Glasses": (1, 2, 3)},},
        },
    }
    ...
    def OnServerInitComplete(self):
        ...
        if not ageSDL:
            PtDebugPrint(
                "nb01EmgrPhase0.OnServerInitComplete:\tNo ageSDL. We're done here."
            )
            return
        ...
        for sdlVar in self.PedestalSDL:
            ...
            PtDebugPrint(
                "nb01EmgrPhase0.OnServerInitComplete(): Pedestal SDL variable {} = {}".format(
                    sdlVar, self._pedestalSDLValues[sdlVar]
                )
            )
        ...
        if self.sceneobject.isLocallyOwned():
            PtDebugPrint(
                "nb01EmgrPhase0.OnServerInitComplete(): Check all pedestal books and stained glasses for sanity."
            )
            for position in self.Pedestals:
                if self.Pedestals[position]:
    ...
    def OnSDLNotify(self, VARname, SDLname, PlayerID, tag):
        ageSDL = PtGetAgeSDL()
        PtDebugPrint(
            "nb01EmgrPhase0.OnSDLNotify():\tVARname: {}, SDLname: {}, tag: {}, value: {}".format(
        ...
        if VARname in self.SimplePagingVars:
            ...
            PtDebugPrint(
                "nb01EmgrPhase0.OnSDLNotify():\t{} = {}".format(
                    VARname, self._pedestalSDLValues[VARname]
                )
            )

        else:
            PtDebugPrint(
                "ERROR: nb01EmgrPhase0.OnSDLNotify():\tVariable '{}' was not recognized.".format(
                    VARname
                )
            )
            return
    ...
    def IManageLinkPedestal(self, position):
        ...
        PtDebugPrint(
            "nb01EmgrPhase0.IManageLinkPedestal(): PedestalPosition = {}".format(
                position
            )
        )
        if position in self.Pedestals:
            ...
            if state:
                if state in pedInfo["Books"]:
                    ...
                    if (
                        self._pedestalSDLValues[pedInfo["sdlGlass"]]
                        not in book["Glasses"]
                    ):
            ...
            PtDebugPrint(
                "nb01EmgrPhase0.IManageLinkPedestal(): No pedestal named '{}'.  Ignoring.".format(
                    position
                )
            )
    ...
    def IPickPedestalBook(self, position):
        if position in self.Pedestals:
            ...
            newBookChoice = random.choice(
                list(self.Pedestals[position]["Books"].keys())
            )
            self._UpdateVaultSDL(self.Pedestals[position]["sdlState"], newBookChoice)

            newBook = self.Pedestals[position]["Books"][newBookChoice]
            PtDebugPrint(
                "nb01EmgrPhase0.IPickPedestalBook():\tSelecting {} as new book for pedestal.".format(
                    newBook["Name"]
                )
            )
            ...
            PtDebugPrint(
                "nb01EmgrPhase0.IPickPedestalBook(): No pedestal named '{}'.  Ignoring.".format(
                    position
                )
            )
    ...
    def IPickPedestalGlass(self, position, bookInfo):
        if position in self.Pedestals:
            ...
            if bookInfo:
                ...
                newGlass = bookInfo["Glasses"][
                    random.randint(0, len(bookInfo["Glasses"]) - 1)
                ]
                PtDebugPrint(
                    "nb01EmgrPhase0.IPickPedestalGlass():\tSelecting stained glass #{} to match book for {}.".format(
                        newGlass, bookInfo["Name"]
                    )
                )
            else:
                PtDebugPrint(
                    "nb01EmgrPhase0.IPickPedestalGlass():\tNo book.  Clearing stained glass."
                )
            self._UpdateVaultSDL(self.Pedestals[position]["sdlGlass"], newGlass)
        else:
            PtDebugPrint(
                "nb01EmgrPhase0.IPickPedestalGlass(): No pedestal named '{}'.  Ignoring.".format(
                    position
                )
            )
    ...
    def _UpdateSimpleStateVar(self, ageSDL, sdlName, value, overwrite=True):
        if isinstance(ageSDL, ptSDLStateDataRecord):
            ...
            if var is None:
                PtDebugPrint(
                    "nb01EmgrPhase0._UpdateSimpleStateVar(): '{}' not found!".format(
                        sdlName
                    )
                )
                return
            ...
            raise TypeError(
                "ageSDL must be a ptSDLStateDataRecord or a ptSimpleStateVariable"
            )

        if not overwrite and var.isUsed():
            PtDebugPrint(
                "nb01EmgrPhase0._UpdateSimpleStateVar(): Skipping update of '{}'".format(
                    sdlName
                ),
        ...
        if var.getType() == PtSDLVarType.kBool:
            ...
            PtDebugPrint(
                "nb01EmgrPhase0._UpdateSimpleStateVar(): '{}' has an unknown type!".format(
                    sdlName
                )
            )
            return
        PtDebugPrint(
            "nb01EmgrPhase0._UpdateSimpleStateVar(): '{}' = {}".format(sdlName, value)
        )
    ...
    def _RandomizeNeighborhood(self):
        ...
        PtDebugPrint(
            "nb01EmgrPhase0._RandomizeNeighborhood(): ---Attempting to Randomize SDL---"
        )
    ...
    def IManageSimplePagingVar(self, VARname):
        ...
        try:
            if ageSDL[VARname][0] == 1:  # Are we paging things in?
                PtDebugPrint(
                    "DEBUG: nb01EmgrPhase0.IManageSimplePagingVar:\tPaging in {}".format(
                        VARname
                    )
                )
                ...
                PtDebugPrint(
                    "DEBUG: nb01EmgrPhase0.IManageSimplePagingVar:\tPaging out {}".format(
                        VARname
                    )
                )
                ...
                PtDebugPrint(
                    "ERROR: nb01EmgrPhase0.IManageSimplePagingVar:\tVariable '{}' had unexpected value of {} (should be a boolean).".format(
                        VARname, ageSDL[VARname][0]
                    )
                )
        except:
            PtDebugPrint(
                "ERROR: nb01EmgrPhase0.IManageSimplePagingVar:\tError managing variable '{}'".format(
                    VARname
                )
            )
            return
