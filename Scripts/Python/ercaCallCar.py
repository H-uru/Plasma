    ...
    def OnSDLNotify(self, VARname, SDLname, playerID, tag):
        ...
        if AgeStartedIn == PtGetAgeName():
            ...
            if VARname == SDLCarPos.value:
                PtDebugPrint(
                    "DEBUG: ercaCallCar.OnSDLNotify():\t VARname:%s, SDLname:%s, tag:%s, value:%d"
                    % (VARname, SDLname, tag, ageSDL[SDLCarPos.value][0])
                )
                byteCarPos = ageSDL[SDLCarPos.value][0]
                if byteCarPos == 2:
                    respHullGate.run(self.key, state="close")
            ...
            if VARname == SDLCarLev.value:
                PtDebugPrint(
                    "DEBUG: ercaCallCar.OnSDLNotify():\t VARname:%s, SDLname:%s, tag:%s, value:%d"
                    % (VARname, SDLname, tag, ageSDL[SDLCarLev.value][0])
                )
                boolCarLev = ageSDL[SDLCarLev.value][0]
                if boolCarLev == 0:
