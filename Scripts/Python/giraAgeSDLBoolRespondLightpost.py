...
class giraAgeSDLBoolRespondLightpost(ptResponder):
    ...
    def OnServerInitComplete(self):
        ...
        if ageSDL[stringVarName.value][0]:
            PtDebugPrint(
                "DEBUG: giraAgeSDLBoolRespondLightpost.OnServerInitComplete:\tRunning true responder on %s, fastforward=%d"
                % (self.sceneobject.getName(), boolFFOnInit.value)
            )
            respBoolTrue.run(self.key, fastforward=boolFFOnInit.value)
        else:
            PtDebugPrint(
                "DEBUG: giraAgeSDLBoolRespondLightpost.OnServerInitComplete:\tRunning false responder on %s, fastforward=%d"
                % (self.sceneobject.getName(), boolFFOnInit.value)
            )
