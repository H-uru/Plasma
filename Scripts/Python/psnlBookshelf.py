...
)  # incoming notifies from the open Linking Book GUI
...
kPublicBooks = (
    ...
    "Cleft",
    "City",
)  # These books cannot be linked to by guests, and cannot be deleted by the owner
...
class psnlBookshelf(ptModifier):
    ...
    def OnServerInitComplete(self):
        ...
        if AgeStartedIn == PtGetAgeName():
            ...
            if not solo and ShelfABoolOperated:
                ...
                PtDebugPrint(
                    "psnlBookshelf.Load():\tShelfABoolOperated=%d but no one else here...correcting"
                    % ShelfABoolOperated
                )
                self.IResetShelf()
    ...
    def OnAgeVaultEvent(self, event, tupdata):
        PtDebugPrint(
            "psnlBookshelf.OnAgeVaultEvent()\t:OnAgeKIEvent recvd. Event=%d and data= "
            % (event),
            tupdata,
        )
        if event == PtVaultCallbackTypes.kVaultConnected:
            ...
            PtDebugPrint(
                "psnlBookshelf.OnAgeVaultEvent()\t: kVaultNodeSaved event (id=%d,type=%d)"
            ...
            PtDebugPrint(
                "psnlBookshelf.OnAgeVaultEvent()\t: kVaultNodeRefAdded event (childID=%d,parentID=%d)"
            ...
            PtDebugPrint(
                "psnlBookshelf.OnAgeVaultEvent()\t: kVaultRemovingNodeRef event (childID=%d,parentID=%d)"
                % (tupdata[0].getChildID(), tupdata[0].getParentID())
            )
            # tupdata is ( ptVaultNodeRef )
            # pass
            ...
            PtDebugPrint(
                "psnlBookshelf.OnAgeVaultEvent()\t: kVaultNodeInitialized event (id=%d,type=%d)",
                (tupdata[0].getID(), tupdata[0].getType()),
            )
            # tupdata is ( ptVaultNode )
            # pass
    ...
    def OnNotify(self, state, id, events):
        ...
        if id == SeekBehavior.id and PtGetLocalAvatar() == PtFindAvatar(events):
            for event in events:
                if (
                    event[0] == kMultiStageEvent and event[1] == 0
        ...
        if id == actLinkingBookGUIPopup.id:
            ...
            for event in events:
                if event[0] == kVariableEvent:
                    PtDebugPrint(
                        "psnlBookshelf: Received a message from the Book GUI: ",
                        event[1],
                    )
                        ...
                        if event[1] == "YesNo" and event[3] == 1:
                            ...
                            if bookAge == "Ahnonay" or bookAge == "AhnonayCathedral":
                                ...
                                for ageInfoChildRef in ageInfoChildren:
                                    ...
                                    if folder and folder.folderGetName() == "AgeData":
                                        ...
                                        for ageDataChildRef in ageDataChildren:
                                            ...
                                            if (
                                                chron
                                                and chron.getName() == "AhnonayVolatile"
                                            ):
                                ...
                                for content in contents:
                                    ...
                                    if (
                                        info
                                        and info.getAgeFilename() == "AhnonayCathedral"
                                    ):
                                        # found our link
                                        PtDebugPrint(
                                    ...
                                    for rkey, rvalue in respDeleteBook.byObject.items():
                                        ...
                                        if parent:
                                            if bookName == parent.getName():
                                                respDeleteBook.run(
                                                    self.key, objectName=rkey
                                                )
                                                BookNumber = linkLibrary.index(bookAge)
                                                ageSDL = PtGetAgeSDL()
                                                ageSDL.setIndex(
                                                    "CurrentPage", BookNumber, 1
                                                )
                                                PtDebugPrint(
        ...
        if id == actBook.id:
            ...
            for event in events:
                if event[0] == kPickedEvent:
                    ...
                    bookName = objBookPicked.getName()
                    PtDebugPrint(
                        "psnlBookshelf.OnNotify():\tplayer picked book named ", bookName
                    )
                    try:
        ...
        if id == actTray.id:
            ...
            if bookAge == "Ahnonay" or bookAge == "AhnonayCathedral":
                ...
                if int(volatile):
                    ...
                    PtYesNoDialog(
                        self.key, PtGetLocalizedString("Personal.Bookshelf.DeleteBook")
                    )
                ...
                if bookAge == "Neighborhood":
                    PtYesNoDialog(
                        ...
                        PtGetLocalizedString(
                            "Personal.Bookshelf.DeleteNeighborhoodBook"
                        ),
                    )
                else:
                    PtYesNoDialog(
    ...
    def IGetLinkFromBook(self, spTitle=None):
        ...
        ageName = self.IGetAgeFromBook()
        PtDebugPrint(
            "psnlBookshelf.IGetLinkFromBook(): before city lookup, ageName = ", ageName
        )
        ...
        if ageName == "city":
            ...
            for age, splist in CityBookAges.items():
                if spTitle in splist:
                    ...
                    break
        PtDebugPrint(
            "psnlBookshelf.IGetLinkFromBook(): after city lookup, ageName = ", ageName
        )
        ...
        if ageName == "Ahnonay":
            PtDebugPrint(
                "psnlBookshelf.IGetLinkFromBook(): Going to Ahnonay... special case."
            )
            return "Ahnonay"
        ...
        PtDebugPrint(
            "psnlBookshelf.IGetLinkFromBook():\tERROR -- couldn't find link to", ageName
        )
    ...
    def IUpdateLocksAndTrays(self):
        ...
        for content in contents:
            ...
            try:
                ...
                PtDebugPrint(
                    "psnlBookshelf.IUpdateLocksAndTrays():\tno matching book for KI's link to:",
                    ageName,
                    "...skipping to next",
                )
                continue
            ...
            if (
                (ageName == "city" or ageName == "BaronCityOffice")
                and PtIsSinglePlayerMode()
            ) or (ageName in CityBookAges.keys()):
                continue
            ...
            if link.getLocked():
                ...
                PtDebugPrint(
                    "psnlBookshelf.IUpdateLocksAndTrays():\tsetting clasp to unlocked: ",
                    lockName,
                )
                for rkey, rvalue in respOpenLock.byObject.items():
                    parent = rvalue.getParentKey()
        ...
        if guid != None:
            try:
                ...
                PtDebugPrint(
                    "psnlBookshelf.IUpdateLocksAndTrays():\tno matching book for KI's link to: Ahnonay... skipping to next"
                )
                return
        ...
        if PAL is not None:
            ...
            for content in contents:
                ...
                try:
                    ...
                    PtDebugPrint(
                        "psnlBookshelf.IUpdateLinks():\tno matching book for KI's link to:",
                        ageName,
                        "...skipping to next",
                    )
                    continue
                ...
                PtDebugPrint(
                    "psnlBookshelf.IUpdateLinks():\tageName: ",
                    ageName,
                    " boolInMyAge: ",
                    boolInMyAge,
                    " getLocked(): ",
    ...
    def ILink(self):
        ...
        if isinstance(link, ptAgeLinkStruct):
            ...
            for sp in spawnPoints:
                if (
                    sp.getTitle() == SpawnPointTitle and sp.getName() == SpawnPointName
                ) or self.CheckForCityBookSpawnPoint(ageName, SpawnPointTitle):
                    PtDebugPrint(
                        "found spawn point: %s, %s" % (sp.getTitle(), sp.getName())
                    )
                    if sp.getName() == "BigRoomLinkInPoint":
                        PtDebugPrint(
                            "oops, found spawnpt for GZ BigRoom, we don't want to link there via city book, so we'll ignore it"
                        )
                        break
                    spnpnt = sp
                    break
        ...
        if vault.inMyPersonalAge():
            if ageName == "Ahnonay":
                ...
                PtDebugPrint(
                    "psnlBookshelf.ILink(): using kChildAgeBook rules for link to: ",
                    ageName,
                )
                ...
                PtDebugPrint(
                    "psnlBookshelf.ILink(): using kOwnedBook rules for link to: ",
                    ageName,
                )
    ...
    def IGetAgeFromBook(self):
        ...
        try:
            ...
            return None
        PtDebugPrint(
            "psnlBookshelf.IGetAgeFromBook():\tpicked book goes to ", linkLibrary[index]
        )
    ...
    def HasCityBook(self):
        ...
        if not vault.amOwnerOfCurrentAge():
            ...
            if GotBook:
                ...
                PtDebugPrint(
                    "psnlBookshelf.HasCityBook(): owner does NOT have city book"
                )
                return 0
