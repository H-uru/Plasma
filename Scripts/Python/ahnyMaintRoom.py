    ...
    def OnFirstUpdate(self):
        ...
        if SphereNum.value == 4:
            ...
            if SphereNum.value != 1 and SphereNum.value != 2 and SphereNum.value != 3:
                PtDebugPrint(
                    "ahnyMaintRoom.OnServerInitComplete():\tERROR---Invalid sphere# set in component.  Disabling clickable."
                )
            ...
            if actingAvatar == PtGetLocalAvatar():
                ...
                if SphereNum.value == 4:
                    ...
                    if boolHubDoor and ageSDL["ahnyCurrentSphere"][0] != 4:
                        PtDebugPrint(
                            "ahnyMaintRoom.OnSDLNotify(): Door is open and we're not going to Sphere 4, so close it."
                        )
                        ageSDL["ahnyHubDoor"] = (0,)
                    elif not boolHubDoor and ageSDL["ahnyCurrentSphere"][0] == 4:
                        PtDebugPrint(
                            "ahnyMaintRoom.OnSDLNotify(): Door is not open and we're going to Sphere 4, so open it."
                        )
