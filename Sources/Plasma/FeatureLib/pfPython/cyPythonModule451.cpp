...
PYTHON_METHOD_DEFINITION(ptModulePackFinder, find_spec, args)
{
    ...
    if (spec) {
        ...
        // We couldn't find the module in our Python pak. This is perfectly fine -
        // we might be an internal build with unpacked Python files. Let other finders
        // try to find it. If no finder can find the module, Python will raise an ImportError.
        PYTHON_RETURN_NONE;
    }
}
...
{
    PyObject* specObj;
    if (!PyArg_ParseTuple(args, "O", &specObj) || false /* fixme type check */) {
        PyErr_SetString(PyExc_TypeError, "create_module expects a ModuleSpec object");
        PYTHON_RETURN_ERROR;
    }
