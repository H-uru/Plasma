...
plKey FindObjectByName(const ST::string& name, int type, const ST::string& ageName, char* statusStr, bool subString)
{
    ...
    if (ageName.is_empty() && plNetClientMgr::GetInstance() != nil)
    {
        ...
        if (!thisAge.is_empty())
        {
            key = plKeyFinder::Instance().StupidSearch(thisAge, ST::string::null, type, name, subString);
            if (key != nil)
            {
                if (statusStr)
    ...
    // Fallback
    key = plKeyFinder::Instance().StupidSearch(ageName, ST::string::null, type, name, subString);
                ...
                "Start the animation" )             // Help string
{
    ...
    msg->SetAnimName(ST::string::null);
