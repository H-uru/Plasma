...
plCGLDevice* plCGLDevice::TryInit(hsWindowHndl window, hsWindowHndl device, const char** error)
{
    IGNORE_WARNINGS_BEGIN("deprecated-declarations")
    ...
    do {
        ...
        if (CGLSetCurrentContext(ctx) != kCGLNoError) {
            *error = "Failed to attach CGL context to surface";
            break;
        }
...
bool plCGLDevice::BeginRender(const char** error)
{
    return true;
}

bool plCGLDevice::EndRender(const char** error)
{
    return true;
}
