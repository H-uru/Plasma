...
plEGLDevice* plEGLDevice::TryInit(hsWindowHndl window, hsWindowHndl device, const char** error)
{
    EGLDisplay display = EGL_NO_DISPLAY;
    EGLContext context = EGL_NO_CONTEXT;
    EGLSurface surface = EGL_NO_SURFACE;
    ...
    do {
        if (!eglBindAPI(EGL_OPENGL_API)) {
            *error = "Could not bind to OpenGL API";
            break;
        }
        ...
        if (display == EGL_NO_DISPLAY) {
            *error = "Could not get the display";
            break;
        }

        if (!eglInitialize(display, nullptr, nullptr)) {
            *error = "Could not initialize the display";
            break;
        }
        ...
        if (!eglChooseConfig(display, config_attrs, &config, 1, &config_count) || config_count != 1) {
            *error = "Could not choose appropriate config";
            break;
        }
        ...
        if (eglMakeCurrent(display, surface, surface, context) == EGL_FALSE) {
            *error = "Failed to attach EGL context to surface";
            break;
        }
...
bool plEGLDevice::BeginRender(const char** error)
{
...
bool plEGLDevice::EndRender(const char** error)
{
