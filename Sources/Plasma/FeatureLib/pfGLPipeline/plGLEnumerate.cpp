...
void plEGLEnumerate(std::vector<hsG3DDeviceRecord>& records, hsDisplayHndl mainDisplay = EGL_DEFAULT_DISPLAY)
{
    ...
    do {
        display = eglGetDisplay(static_cast<EGLNativeDisplayType>(mainDisplay));
        ...
        if (fillDeviceRecord(devRec, ST_LITERAL("EGL OpenGL API"), mainDisplay))
...
void plWGLEnumerate(std::vector<hsG3DDeviceRecord>& records, hsDisplayHndl mainDisplay = INVALID_HANDLE_VALUE)
{
    ...
    do {
        ...
        hsG3DDeviceRecord devRec;
        if (fillDeviceRecord(devRec, ST_LITERAL("opengl32.dll"), mainDisplay))
            records.emplace_back(devRec);
    } while (0);
...
void plCGLEnumerate(std::vector<hsG3DDeviceRecord>& records, CGDirectDisplayID mainDisplay = kCGNullDirectDisplay)
{
