...
plWGLDevice* plWGLDevice::TryInit(hsWindowHndl window, hsWindowHndl device, const char** error)
{
    ...
    do {
        ...
        int format = ChoosePixelFormat(dc, &pfd);
        if (!format) {
            *error = "Could not find appropriate pixel config";
            break;
        }

        if (!SetPixelFormat(dc, format, &pfd)) {
            *error = "Could not set appropriate pixel config";
            break;
        }
        ...
        if (!wglMakeCurrent(dc, ctx)) {
            *error = "Failed to attach WGL context to surface";
            break;
        }
...
bool plWGLDevice::BeginRender(const char** error)
{
...
bool plWGLDevice::EndRender(const char** error)
{
