...
// Ported from ps_GrassShader (Grass.metal:112-125).
...
void main()
{
    vec4 color = texture(sampler2D(textures2D[0], samplers[0]), inTexCoord.xy) * inColor;

    // Blades are cut out of the texture, so almost-transparent is nothing.
    if (color.a <= 0.1)
        discard;
