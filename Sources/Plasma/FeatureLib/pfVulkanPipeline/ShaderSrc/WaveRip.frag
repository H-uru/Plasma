...
// Ported from ps_WaveRip (WaveRip.metal:265-273).
...
void main()
{
    // A ripple is a single decal, so its texture is clamped: repeating it would
    // tile the ripple across the whole span.
    outColor = texture(sampler2D(textures2D[0], samplers[kSamplerClampBoth]), inTexCoord) * inColor;
}
