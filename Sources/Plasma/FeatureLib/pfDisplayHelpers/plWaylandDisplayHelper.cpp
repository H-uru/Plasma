...
void plWaylandDisplayHelper::output_handle_mode(void* data,
        wl_output* wl_output, uint32_t flags, int32_t width, int32_t height,
        int32_t refresh)
{
...
void plWaylandDisplayHelper::global_handler(void* data, wl_registry* registry,
        uint32_t id, const char* interface, uint32_t version)
{
    ...
    if (strcmp(interface, (*__wl_output_interface).name) == 0) {
        ...
        self->fCurrentOutput = reinterpret_cast<wl_output*>(wl_registry_bind(
            registry, id, &__wl_output_interface,
            std::min<uint32_t>((*__wl_output_interface).version, version)
        ));
...
std::vector<plDisplayMode> plWaylandDisplayHelper::GetSupportedDisplayModes(
