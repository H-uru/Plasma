...
size_t hsQueueStream::Read(size_t byteCount, void* buffer)
{
    ...
    size_t limit = fWriteCursor >= fReadCursor ? fWriteCursor : fSize;
    size_t length = std::min(limit-fReadCursor, byteCount);
...
size_t hsQueueStream::Write(size_t byteCount, const void* buffer)
{
    ...
    size_t length = std::min(fSize-fWriteCursor, byteCount);
...
{
    size_t limit = fWriteCursor >= fReadCursor ? fWriteCursor : fSize;
    size_t length = std::min(limit-fReadCursor, static_cast<size_t>(deltaByteCount));
    fReadCursor += length;
...
void hsBufferedStream::Skip(uint32_t delta)
{
    ...
    else
    {
        ...
        if (fBufferLen > 0)
        {
            uint32_t newBufferPos = (fPosition % kBufferSize) + delta;
