...
bool hsBitIterator::IAdvanceVec()
{
    ...
    while ((uint32_t(++fCurrVec) < fBits.fNumBitVectors) && !fBits.fBitVectors[fCurrVec])
        ;

    return uint32_t(fCurrVec) < fBits.fNumBitVectors;
}
