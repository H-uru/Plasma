...
class pfPasswordStoreTest : public testing::Test {
protected:
    void SetUp() override {
        SetServerDisplayName(ST_LITERAL("pfPasswordStoreTest"));
    }
};

TEST_F(pfPasswordStoreTest, pfPasswordStore_service)
{
...
#if defined(HS_BUILD_FOR_WIN32)
TEST_F(pfPasswordStoreTest, pfWin32PasswordStore_storing_and_retrieval)
{
...
TEST_F(pfPasswordStoreTest, pfApplePasswordStore_storing_and_retrieval)
{
...
TEST_F(pfPasswordStoreTest, pfUnixPasswordStore_storing_and_retrieval)
{
    if (!getenv("DBUS_SESSION_BUS_ADDRESS")) {
        // This test case fails in CI because of dbus session stuff, so just skip it
        GTEST_SKIP() << "Skipped due to lack of DBus session";
        return;
    }
...
TEST_F(pfPasswordStoreTest, pfFilePasswordStore_storing_and_retrieval)
{
